...
tea <- subset(tea, TBI_S > 0)
tea[tea$Tea_Type == "Rooibos" & tea$TBI_S > 0.5,]$TBI_S <- NA
...
# soil temperature#
var.list <- c("TBI_S", "TBI_S_Day", "k", "TBI_k", "TBI_S")
...
season_narm_r <- season_narm_r %>%
  ...
  dplyr::mutate(NObsPlot = length(TBI_S))
...
load("users/hthomas/tea/Stan_outputs/airtemp_fits_summer_TBI_S.Rdata")
...
ggplot() +
  ...
  geom_point(
    data = season_narm_r,
    aes(x = airtemp_mean + airtemp_cent_amount, y = TBI_S),
    colour = "#00b100",
    pch = 16 ,
    alpha = 0.6
  ) +
  ...
  # scale_colour_manual(values = c("#00b100","#00b100"), name = "Tea Type")+
  # scale_fill_manual(values = c("#00b100","#00b100"), name = "Tea Type")+
  # scale_linetype_manual(values = c("dashed","solid"), name = "airtemp", labels = c("low","high"))+
  labs(x = "Soil temp. (°C)", y = "Mass TBI_S (%)") +
...
(
  air <- ggplot() +
    ...
    geom_line(
      data = predsout.space_air,
      aes(x = airtemp + airtemp_cent_amount, y = mean * 100),
      colour = "#00b100",
      alpha = 1,
      lwd = 1.5
    ) +
...
load("users/hthomas/tea/Stan_outputs/soiltemp_fits_summer_TBI_S.Rdata")
...
(
  soil <- ggplot() +
    ...
    geom_line(
      data = predsout.space_soil,
      aes(x = soiltemp + soiltemp_cent_amount, y = mean * 100),
      colour = "#00b100",
      alpha = 1,
      lwd = 1.5
    ) +
    ...
    labs(x = "Site summer soil temp. (°C)",
         y = expression(
           paste("Stabilisation factor (", italic(S), ")")
         )) +
    theme(legend.position = "none") +
...
load("users/hthomas/tea/Stan_outputs/moisture_fits_summer_TBI_S.Rdata")
...
(
  moisture <- ggplot() +
    ...
    geom_line(
      data = predsout.space_moisture,
      aes(x = moisture + moisture_cent_amount, y = mean * 100),
      colour = "#00b100",
      alpha = 1,
      lwd = 1.5
    ) +
...
load("users/hthomas/tea/Stan_outputs/CHELSA_summer_temp_fits_summer_TBI_S.Rdata")
...
(
  CHELSA_summer_temp <- ggplot() +
    ...
    labs(x = "Gridded summer air temp. (°C)",
         y = expression(
           paste("Stabilisation factor (", italic(S), ")")
         )) +
    theme(legend.position = "none") +
...
load("users/hthomas/tea/Stan_outputs/CHELSA_summer_precip_fits_summer_TBI_S.Rdata")
...
(
  CHELSA_summer_precip <- ggplot() +
    ...
    labs(x = "Gridded summer precip. (mm)",
         y = expression(
           paste("Stabilisation factor (", italic(S), ")")
         )) +
    theme(legend.position = "none") +
...
load("users/hthomas/tea/Stan_outputs/ESA_moisture_fits_summer_TBI_S.Rdata")
...
(
  ESA_moisture <- ggplot() +
    ...
    geom_line(
      data = predsout.space_ESA_moisture,
      aes(x = ESA_moisture + ESA_moisture_cent_amount, y = mean * 100),
      colour = "#00b100",
      alpha = 1,
      lwd = 1.5
    ) +
    ...
    labs(x = "Gridded summer soil moist. (%)",
         y = expression(
           paste("Stabilisation factor (", italic(S), ")")
         )) +
    theme(legend.position = "none") +
...
pdf(file = "users/hthomas/tea/figures/Env_biome_alternative_S.pdf",
    width = 10.5,
    height = 7.5)
grid.arrange(air,
             soil,
