...
ggplot(plot_means[plot_means$Season=="Winter",],aes(S,k))+
  geom_point(aes(colour=factor(Region)),alpha=0.5)+
  theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  ggtitle("S vs k - Winter")
...
#Crop for non-summer
chelsa_winter_tundra<-crop(chelsa_winter, extent(-180.000, 180, 45, 84))
chelsa_precip_winter_tundra<-crop(chelsa_precip_winter, extent(-180.000, 180, 45, 84))
...
ESA_winter<-raster("scripts/users/hthomas/ESA_Soil_Moisture/1978_2015_mean.tif")

#Add to tea object
tea$ESA_moisture<-extract(ESA_winter,tea_coords)*100
...
winter_dens<-ggplot(winter, aes(x=Loss*100, y = ..density..,fill=factor(Tea_Type)))+
  geom_histogram(alpha=0.8,position = 'identity')+
  geom_density(alpha=0.25, adjust=3)+
  scale_x_continuous(limits = c(0,100))+
  scale_fill_manual(values = c("darkgreen", "red"),name="Tea Type")+
  theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  labs(y="Desnity",x="% Mass Loss")+
  ggtitle("Winter incubations")

grid.arrange(all,ambs,winter_dens)
...
pdf(file="scripts/users/hthomas/Output_Images/Tea/tea_correlations_winter.pdf", width = 8, height = 5)
...
#Seasons: i) Winter, ii) Winter, iii) Year

#Set season
season.list <- c("winter", "winter", "year")
...
for(i in 1:5){
  ...
  season_narm<-get(season.list[2]) %>%
    filter(is.finite(get(season.list[2])[,var.num]),is.finite(airtemp_mean))
  
  #Define interest variable
  names(season_narm)[var.num]<-"var"
  
  #MCMC model
  MC_LM_air <- MCMCglmm(var ~ Tea_Type + airtemp_mean + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000) #Run
  ...
  #Run for green
  MC_ML_air_g<-MCMCglmm(var ~ airtemp_mean + Days,  data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  summary(MC_ML_air_g)
  ...
  #Run for rooibos
  MC_ML_air_r<-MCMCglmm(var ~ airtemp_mean + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  summary(MC_ML_air_r)
  ...
  plotting_data<-get(season.list[2]) #copy dataset
  ...
  assign(paste("air_fig",var.list[i],season.list[2],sep="_"),sum_air)
  
  #b) SOIL TEMP ONLY#################################################
  
  #Remove na values from season object
  season_narm<-get(season.list[2]) %>%
    filter(is.finite(get(season.list[2])[,var.num]),is.finite(soiltemp_mean))
  
  #Define interest variable
  names(season_narm)[var.num]<-"var"
  
  #Create model
  MC_LM_soil <- MCMCglmm(var ~ Tea_Type + soiltemp_mean + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000) #Run
  ...
  #Run for green
  MC_ML_soil_g<-MCMCglmm(var ~ soiltemp_mean + Days, data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  ...
  #Run for rooibos
  MC_ML_soil_r<-MCMCglmm(var ~ soiltemp_mean + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  ...
  assign(paste("soil_t_fig",var.list[i],season.list[2],sep="_"),sum_soil_t)
  ...
  #Remove na values from season object
  season_narm<-get(season.list[2]) %>%
    filter(is.finite(get(season.list[2])[,var.num]),is.finite(moisture_mean))
  
  #Define interest variable
  names(season_narm)[var.num]<-"var"
  
  #Create model
  MC_LM_moisture <- MCMCglmm(var ~ Tea_Type + moisture_mean + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000)
  ...
  #Run for green
  MC_ML_moisture_g<-MCMCglmm(var ~ moisture_mean + Days, data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  ...
  #Run for rooibos
  MC_ML_moisture_r<-MCMCglmm(var ~ moisture_mean + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  ...
  assign(paste("soil_m_fig",var.list[i],season.list[2],sep="_"),sum_soil_m)
  
  #d) CHELSA AIR###################################################
  
  #Remove na values from season object
  season_narm<-get(season.list[2]) %>%
    filter(is.finite(get(season.list[2])[,var.num]))
  
  #Define interest variable
  names(season_narm)[var.num]<-"var"
  
  #Create model
  MC_LM_CHELSA_temp <- MCMCglmm(var ~ Tea_Type + CHELSA_winter_temp  + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000)
  summary(MC_LM_CHELSA_temp)
  
  #Calculate mean burial length
  mean_burial<-mean(season_narm$Days)
  min_CH_temp<-min(season_narm$CHELSA_winter_temp,na.rm=TRUE)
  max_CH_temp<-max(season_narm$CHELSA_winter_temp,na.rm=TRUE)
  
  #Run for green
  MC_ML_CH_T_g<-MCMCglmm(var ~ CHELSA_winter_temp  + Days, data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  summary(MC_ML_CH_T_g)
  
  #Model preditions
  mm.green <- expand.grid(CHELSA_winter_temp = seq(min_CH_temp, max_CH_temp, 0.1), Days = rep(mean_burial), var = 0)  # Create a blank dataset with the years we want
  mm <- model.matrix(terms(var ~ CHELSA_winter_temp + Days), mm.green)  # Create matrix of relevant effect sizes
  ...
  #Run for rooibos
  MC_ML_CH_T_r<-MCMCglmm(var ~ CHELSA_winter_temp  + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  summary(MC_ML_CH_T_r)
  
  #Model preditions
  mm.red <- expand.grid(CHELSA_winter_temp = seq(min_CH_temp, max_CH_temp, 0.1), Days = rep(mean_burial), var = 0)  # Create a blank dataset with the years we want
  mm <- model.matrix(terms(var ~ CHELSA_winter_temp + Days), mm.red)  # Create matrix of relevant effect sizes
  ...
  #pdf(file="scripts/users/hthomas/Output_Images/Tea/moisture_temp_MCMC.pdf", width = 3, height = 3)
  (sum_CH_T <- ggplot()+
      stat_smooth(data=plotting_data[plotting_data$Tea_Type=="Green",],aes(CHELSA_winter_temp,var*100),method="lm",colour="forestgreen",size=0.6, fill = "darkgreen", alpha = 0.3)+
      stat_smooth(data=plotting_data[plotting_data$Tea_Type=="Rooibos",],aes(CHELSA_winter_temp,var*100),method="lm",colour="red",size=0.6, fill = "red", alpha = 0.3)+
      #geom_ribbon(data = mm.red, mapping = aes(x = CHELSA_winter_temp, ymin = plo*100, ymax = phi*100), fill = "red3", alpha = 0.3) +
      #geom_line(data = mm.red, mapping = aes(x = CHELSA_winter_temp, y=var*100), colour="red3", size=1) +
      #geom_ribbon(data = mm.green, mapping = aes(x = CHELSA_winter_temp, ymin = plo*100, ymax = phi*100), fill = "darkgreen", alpha = 0.3) +
      #geom_line(data = mm.green, mapping = aes(x = CHELSA_winter_temp,y=var*100), colour="darkgreen", size=1) +
      geom_point(data=plotting_data[plotting_data$Tea_Type=="Green",],aes(CHELSA_winter_temp,var*100),colour="green3",alpha=0.3,pch=16)+
      geom_point(data=plotting_data[plotting_data$Tea_Type=="Rooibos",],aes(CHELSA_winter_temp,var*100),colour="indianred1",alpha=0.3,pch=16)+
      theme_bw()+
      theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
      scale_colour_manual(values = c("darkgreen", "red"),name="Tea Type")+
      labs(y="% Mass Loss",x="CHELSA Winter Temperature (°C)")+
      ggtitle("CHELSA Winter Temperature"))
  #dev.off()
  
  assign(paste("CH_t_fig",var.list[i],season.list[2],sep="_"),sum_CH_T)
  ...
  #Create model
  MC_LM_CHELSA_precip <- MCMCglmm(var ~ Tea_Type + CHELSA_winter_precip  + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000)
  summary(MC_LM_CHELSA_precip)
  
  #Calculate mean burial length
  mean_burial<-mean(season_narm$Days)
  min_CH_precip<-min(season_narm$CHELSA_winter_precip,na.rm=TRUE)
  max_CH_precip<-max(season_narm$CHELSA_winter_precip,na.rm=TRUE)
  
  #Run for green
  MC_ML_CH_P_g<-MCMCglmm(var ~ CHELSA_winter_precip  + Days, data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  summary(MC_ML_CH_P_g)
  
  #Model preditions
  mm.green <- expand.grid(CHELSA_winter_precip = seq(min_CH_precip, max_CH_precip, 0.1), Days = rep(mean_burial), var = 0)  # Create a blank dataset with the years we want
  mm <- model.matrix(terms(var ~ CHELSA_winter_precip + Days), mm.green)  # Create matrix of relevant effect sizes
  ...
  #Run for rooibos
  MC_ML_CH_P_r<-MCMCglmm(var ~ CHELSA_winter_precip  + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  summary(MC_ML_CH_P_r)
  
  #Model preditions
  mm.red <- expand.grid(CHELSA_winter_precip = seq(min_CH_precip, max_CH_precip, 0.1), Days = rep(mean_burial), var = 0)  # Create a blank dataset with the years we want
  mm <- model.matrix(terms(var ~ CHELSA_winter_precip + Days), mm.red)  # Create matrix of relevant effect sizes
  ...
  #pdf(file="scripts/users/hthomas/Output_Images/Tea/moisture_temp_MCMC.pdf", width = 3, height = 3)
  (sum_CH_P <- ggplot()+
      stat_smooth(data=plotting_data[plotting_data$Tea_Type=="Green",],aes(CHELSA_winter_precip,var*100),method="lm",colour="forestgreen",size=0.6, fill = "darkgreen", alpha = 0.3)+
      stat_smooth(data=plotting_data[plotting_data$Tea_Type=="Rooibos",],aes(CHELSA_winter_precip,var*100),method="lm",colour="red",size=0.6, fill = "red", alpha = 0.3)+
      #geom_ribbon(data = mm.red, mapping = aes(x = CHELSA_winter_precip, ymin = plo*100, ymax = phi*100), fill = "red3", alpha = 0.3) +
      #geom_line(data = mm.red, mapping = aes(x = CHELSA_winter_precip, y=var*100), colour="red3", size=1) +
      #geom_ribbon(data = mm.green, mapping = aes(x = CHELSA_winter_precip, ymin = plo*100, ymax = phi*100), fill = "darkgreen", alpha = 0.3) +
      #geom_line(data = mm.green, mapping = aes(x = CHELSA_winter_precip,y=var*100), colour="darkgreen", size=1) +
      geom_point(data=plotting_data[plotting_data$Tea_Type=="Green",],aes(CHELSA_winter_precip,var*100),colour="green3",alpha=0.3,pch=16)+
      geom_point(data=plotting_data[plotting_data$Tea_Type=="Rooibos",],aes(CHELSA_winter_precip,var*100),colour="indianred1",alpha=0.3,pch=16)+
      theme_bw()+
      theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
      scale_colour_manual(values = c("darkgreen", "red"),name="Tea Type")+
      labs(y="% Mass Loss",x="CHELSA Winter Precipitation (mm)")+
      ggtitle("CHELSA Winter Precipitation"))
  #dev.off()
  
  assign(paste("CH_p_fig",var.list[i],season.list[2],sep="_"),sum_CH_P)
  
  #f) ESA Moisture###################################################
  
  #Remove na values from season object
  season_narm<-get(season.list[2]) %>%
    filter(is.finite(get(season.list[2])[,var.num]),is.finite(ESA_moisture))
  ...
  #Create model
  MC_LM_ESA_moist <- MCMCglmm(var ~ Tea_Type + ESA_moisture  + Days, data = season_narm, family = "gaussian", pr=TRUE, nitt = 55000, burnin = 5000)
  ...
  #Run for green
  MC_ML_ESA_moist_g<-MCMCglmm(var ~ ESA_moisture  + Days, data = season_narm[season_narm$Tea_Type=="Green",], nitt = 55000, burnin = 5000)
  ...
  #Run for rooibos
  MC_ML_ESA_moist_r<-MCMCglmm(var ~ ESA_moisture  + Days, data = season_narm[season_narm$Tea_Type=="Rooibos",], nitt = 55000, burnin = 5000)
  ...
  assign(paste("ESA_moist_fig",var.list[i],season.list[2],sep="_"),sum_ESA_moist)
}

grid.arrange(air_fig_Loss_winter, soil_t_fig_Loss_winter, soil_m_fig_Loss_winter, CH_p_fig_Loss_winter, CH_t_fig_Loss_winter, ESA_moist_fig_Loss_winter)


#Overall model (Loss - winter)####

winter_narm<-winter%>%
  filter(is.finite(Loss),is.finite(soiltemp_mean),is.finite(moisture_mean), is.finite(Days))

Loss_winter<-MCMCglmm(scale(Loss) ~ Tea_Type + scale(soiltemp_mean) + scale(moisture_mean),  data = winter_narm, nitt = 55000, burnin = 5000)
summary(Loss_winter)

#NB - I'm not sure whether it is better to use days as a fixed or random effect

Loss_winter_fixefs<-as.data.frame(abs(summary(Loss_winter)$solution[,1]))
names(Loss_winter_fixefs)<-"Effect_Size"
rownames(Loss_winter_fixefs)<-c("Intercept","Tea Type","Soil Temperature","Soil Moisture")
Loss_winter_fixefs$Variable<-rownames(Loss_winter_fixefs)
Loss_winter_fixefs$lower<-abs(summary(Loss_winter)$solution[,2])
Loss_winter_fixefs$upper<-abs(summary(Loss_winter)$solution[,3])

pdf(file="scripts/users/hthomas/Output_Images/Tea/Effect_sizes_winter.pdf", width = 4, height = 3)
(winter_effects<-ggplot(Loss_winter_fixefs[-1,], aes(Variable,Effect_Size))+
  geom_errorbar(data=Loss_winter_fixefs[-1,], mapping=aes(x=Variable, ymin=upper, ymax=lower), linetype=1, width=0, size=1)+
  coord_flip()+
...
ggplot(Loss_year_fixefs[-1,], aes(Variable,Effect_Size))+
  ...
  theme_bw()+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  ggtitle("Effect Sizes (Winter Mass Loss)")+
  labs(y = "Standardised Effect Size")


#############With CHELSA data##########################################################

#Chelsa winter data vs measured winter data - air####

#Remove NAs
winter_narm<-winter%>%
  filter(is.finite(CHELSA_winter_temp),is.finite(airtemp_mean))

#Find only unique plot temperatures
winter_narm_unique<-winter_narm[!duplicated(winter_narm[c("CHELSA_winter_temp","airtemp_mean")]),]

#Take extent
min_air<-min(winter_narm_unique$airtemp_mean,na.rm=TRUE)
max_air<-max(winter_narm_unique$airtemp_mean,na.rm=TRUE)

#Run model
lm_air_chelsa_winter<-lmer(CHELSA_winter_temp ~ airtemp_mean + (1|Region), data=winter_narm_unique)
#lm_air_chelsa_winter<-lm(CHELSA_winter_temp ~ airtemp_mean, data=winter_narm_unique)
summary(lm_air_chelsa_winter); r.squaredGLMM(lm_air_chelsa_winter)

# Predictions
mm.air <- expand.grid(airtemp_mean = seq(min_air, max_air, 0.1), CHELSA_winter_temp = 0)  # Create a blank dataset with the years we want
mm <- model.matrix(terms(lm_air_chelsa_winter), mm.air)  # Create matrix of relevant effect sizes
mm.air$CHELSA_winter_temp <- mm %*% fixef(lm_air_chelsa_winter)  # Calculate height based on the relevant effect sizes
pvar.mm.air <- diag(mm %*% tcrossprod(vcov(lm_air_chelsa_winter), mm))
mm.air <- data.frame(mm.air, plo = mm.air$CHELSA_winter_temp-1.96*sqrt(pvar.mm.air),
                     phi = mm.air$CHELSA_winter_temp+1.96*sqrt(pvar.mm.air))  # Add errors

(air<-ggplot(winter_narm_unique, aes(airtemp_mean, CHELSA_winter_temp))+
...
#Chelsa winter data vs measured winter data - soil####

#Remove NAs
winter_narm<-winter%>%
  filter(is.finite(CHELSA_winter_temp),is.finite(soiltemp_mean))

#Find only unique plot temperatures
winter_narm_unique<-winter_narm[!duplicated(winter_narm[c("CHELSA_winter_temp","soiltemp_mean")]),]

#Take extent
min_soil<-min(winter_narm_unique$soiltemp_mean,na.rm=TRUE)
max_soil<-max(winter_narm_unique$soiltemp_mean,na.rm=TRUE)

#Run model
lm_soil_chelsa_winter<-lmer(CHELSA_winter_temp ~ soiltemp_mean + (1|Region), data=winter_narm_unique)
#lm_soil_chelsa_winter<-lm(CHELSA_winter_temp ~ soiltemp_mean, data=winter_narm_unique)
summary(lm_soil_chelsa_winter); r.squaredGLMM(lm_soil_chelsa_winter)

#Predictions
mm.soil <- expand.grid(soiltemp_mean = seq(min_soil, max_soil, 0.1), CHELSA_winter_temp = 0)  # Create a blank dataset with the years we want
mm <- model.matrix(terms(lm_soil_chelsa_winter), mm.soil)  # Create matrix of relevant effect sizes
mm.soil$CHELSA_winter_temp <- mm %*% fixef(lm_soil_chelsa_winter)  # Calculate height based on the relevant effect sizes
pvar.mm.soil <- diag(mm %*% tcrossprod(vcov(lm_soil_chelsa_winter), mm))
mm.soil <- data.frame(mm.soil, plo = mm.soil$CHELSA_winter_temp-1.96*sqrt(pvar.mm.soil),
                      phi = mm.soil$CHELSA_winter_temp+1.96*sqrt(pvar.mm.soil))  # Add errors

(soil<-ggplot(winter_narm_unique, aes(soiltemp_mean, CHELSA_winter_temp))+
...
#Chelsa winter data vs measured winter data - moisture####

#Remove NAs
winter_narm<-winter%>%
  filter(is.finite(CHELSA_winter_precip),is.finite(moisture_mean))

#Find only unique plot temperatures
winter_narm_unique<-winter_narm[!duplicated(winter_narm[c("CHELSA_winter_precip","moisture_mean")]),]

#Take extent
min_moisture<-min(winter_narm_unique$moisture_mean,na.rm=TRUE)
max_moisture<-max(winter_narm_unique$moisture_mean,na.rm=TRUE)

#Run model
#lm_moisture_chelsa_winter<-lmer(CHELSA_winter_precip ~ moisture_mean + (1|Region), data=winter_narm_unique)
lm_moisture_chelsa_winter<-lm(CHELSA_winter_precip ~ moisture_mean, data=winter_narm_unique)
summary(lm_moisture_chelsa_winter); r.squaredGLMM(lm_moisture_chelsa_winter)

# Predictions
mm.moisture <- expand.grid(moisture_mean = seq(min_moisture, max_moisture, 0.1), CHELSA_winter_precip = 0)  # Create a blank dataset with the years we want
mm <- model.matrix(terms(lm_moisture_chelsa_winter), mm.moisture)  # Create matrix of relevant effect sizes
mm.moisture$CHELSA_winter_precip <- mm %*% fixef(lm_moisture_chelsa_winter)  # Calculate height based on the relevant effect sizes
pvar.mm.moisture <- diag(mm %*% tcrossprod(vcov(lm_moisture_chelsa_winter), mm))
mm.moisture <- data.frame(mm.moisture, plo = mm.moisture$CHELSA_winter_precip-1.96*sqrt(pvar.mm.moisture),
                          phi = mm.moisture$CHELSA_winter_precip+1.96*sqrt(pvar.mm.moisture))  # Add errors

(precip<-ggplot(winter_narm_unique, aes(moisture_mean, CHELSA_winter_precip))+
...
#ESA winter data vs measured winter data - moisture####
...
#Find only unique plot temperatures
winter_narm_unique<-winter_narm[!duplicated(winter_narm[c("ESA_moisture","moisture_mean")]),]
...
#Run model
#lm_moisture_ESA_winter<-lmer(ESA_moisture ~ moisture_mean + (1|Region), data=winter_narm_unique)
lm_moisture_ESA_winter<-lm(ESA_moisture ~ moisture_mean, data=winter_narm_unique)
summary(lm_moisture_ESA_winter); r.squaredGLMM(lm_moisture_ESA_winter)
...
# mm <- model.matrix(terms(lm_moisture_ESA_winter), mm.moisture)  # Create matrix of relevant effect sizes
# mm.moisture$ESA_moisture <- mm %*% fixef(lm_moisture_ESA_winter)  # Calculate height based on the relevant effect sizes
# pvar.mm.moisture <- diag(mm %*% tcrossprod(vcov(lm_moisture_ESA_winter), mm))
...
(moisture<-ggplot(winter_narm_unique, aes(moisture_mean, ESA_moisture))+
    #geom_ribbon(data = mm.moisture, mapping = aes(x = moisture_mean, ymin = plo, ymax = phi), alpha = 0.3) +
...
#Combine figures######
pdf(file="scripts/users/hthomas/Output_Images/Tea/Env_vars_winter.pdf", width = 12.5, height = 10)
grid.arrange(air_fig_Loss_winter,soil_t_fig_Loss_winter,soil_m_fig_Loss_winter,soil_m_fig_Loss_winter,
             CH_t_fig_Loss_winter,CH_t_fig_Loss_winter,CH_p_fig_Loss_winter,ESA_moist_fig_Loss_winter,
             air,soil,precip,moisture,ncol=4)
dev.off()


#Create effect-size figure
winter_dens<-winter_dens +
  theme(legend.position = "none")
winter_dens <- ggplot_gtable(ggplot_build(winter_dens))

winter_effects <- ggplot_gtable(ggplot_build(winter_effects))

maxWidth = unit.pmax(winter_dens$widths[2:3], winter_effects$widths[2:3])

winter_dens$widths[2:3] <- maxWidth
winter_effects$widths[2:3] <- maxWidth

pdf(file="scripts/users/hthomas/Output_Images/Tea/Env_vars_effects_winter.pdf", width = 3, height = 3)
grid.arrange(winter_dens,winter_effects,nrow=2)
dev.off()
...
ambient_normal <- ambient %>%
  ...
  mutate(Loss_normal = scale(Loss), 
         ...
         CHELSA_winter_temp_normal = scale(CHELSA_winter_temp),
         CHELSA_winter_precip_normal = scale(CHELSA_winter_precip),
         ESA_moisture_normal = scale(ESA_moisture))

winter_normal<-subset(ambient_normal,Season=="Winter")

#Run within-site models####≈
#a) Air temp
season_narm<-winter_normal %>%
...
(air_normal_fig<-ggplot(winter_normal, aes(airtemp_mean_normal, Loss_normal))+
...
(soil_normal_fig<-ggplot()+
    geom_point(data = winter_normal, aes(soiltemp_mean_normal, Loss_normal,colour = factor(Region)), alpha = 0.5, pch = 16)+
...
(moisture_normal_fig<-ggplot()+
    geom_point(data = winter_normal, aes(moisture_mean_normal, Loss_normal,colour = factor(Region)), alpha = 0.5, pch = 16)+
    ...
    theme_classic()+
    stat_smooth(data = winter_normal, aes(moisture_mean_normal, Loss_normal), method = "lm", alpha = 0.3, fill = "dodgerblue", size=1, colour = "dodgerblue")+
...
season_narm<-winter_normal %>%
  filter(is.finite(Loss_normal),is.finite(CHELSA_winter_temp_normal))

CH_t_normal<-lmer(Loss_normal ~ CHELSA_winter_temp_normal + (1|Region/Site/Plot), data = season_narm)
#CH_t_normal<-MCMCglmm(Loss_normal ~ CHELSA_winter_temp_normal, random = ~Region, data = season_narm, family = "gaussian", pr=TRUE, nitt = 10000, burnin = 1000)
summary(CH_t_normal)

#Take extent
min_CH_t<-min(season_narm$CHELSA_winter_temp_normal,na.rm=TRUE)
max_CH_t<-max(season_narm$CHELSA_winter_temp_normal,na.rm=TRUE)

# #Model predictions
# mm.CH_t <- expand.grid(CHELSA_winter_temp_normal = seq(min_CH_t, max_CH_t, 0.1), Loss_normal = 0)  # Create a blank dataset with the years we want
# mm <- model.matrix(terms(Loss_normal ~ CHELSA_winter_temp_normal), mm.CH_t)  # Create matrix of relevant effect sizes
...
(CH_t_normal_fig<-ggplot()+
    geom_point(data = winter_normal, aes(CHELSA_winter_temp_normal, Loss_normal,colour = factor(Region)), alpha = 0.5, pch = 16)+
    geom_ribbon(data = mm.CH_t, mapping = aes(x = CHELSA_winter_temp_normal, ymin = plo, ymax = phi), alpha = 0.3, fill = "red") +
    geom_line(data = mm.CH_t, mapping = aes(x = CHELSA_winter_temp_normal, y=Loss_normal), size=1, colour = "red") +
    theme_classic()+
    stat_smooth(method = "lm")+
...
#e) CHELSA Precip
season_narm<-winter_normal %>%
  filter(is.finite(Loss_normal),is.finite(CHELSA_winter_precip_normal))

CH_p_normal<-lmer(Loss_normal ~ CHELSA_winter_precip_normal + (1|Region/Site/Plot), data = season_narm)
#CH_p_normal<-MCMCglmm(Loss_normal ~ CHELSA_winter_precip_normal, random = ~Region, data = season_narm, family = "gaussian", pr=TRUE, nitt = 10000, burnin = 1000)
summary(CH_p_normal)

#Take extent
min_CH_p<-min(season_narm$CHELSA_winter_precip_normal,na.rm=TRUE)
max_CH_p<-max(season_narm$CHELSA_winter_precip_normal,na.rm=TRUE)

# #Model predictions
# mm.CH_p <- expand.grid(CHELSA_winter_precip_normal = seq(min_CH_p, max_CH_p, 0.1), Loss_normal = 0)  # Create a blank dataset with the years we want
# mm <- model.matrix(terms(Loss_normal ~ CHELSA_winter_precip_normal), mm.CH_p)  # Create matrix of relevant effect sizes
...
(CH_p_normal_fig<-ggplot()+
    geom_point(data = winter_normal, aes(CHELSA_winter_precip_normal, Loss_normal,colour = factor(Region)), alpha = 0.5, pch = 16)+
    geom_ribbon(data = mm.CH_p, mapping = aes(x = CHELSA_winter_precip_normal, ymin = plo, ymax = phi), alpha = 0.3, fill = "dodgerblue") +
    geom_line(data = mm.CH_p, mapping = aes(x = CHELSA_winter_precip_normal, y=Loss_normal), size=1, colour = "dodgerblue") +
    theme_classic()+
    stat_smooth(method = "lm")+
...
ESA_m_normal<-lmer(Loss_normal ~ ESA_moisture_normal + (1|Region/Site/Plot), data = season_narm)
...
(ESA_m_normal_fig<-ggplot()+
    geom_point(data = winter_normal, aes(ESA_moisture_normal, Loss_normal,colour = factor(Region)), alpha = 0.5, pch = 16)+
...
#Arrange plots
pdf(file="scripts/users/hthomas/Output_Images/Tea/normalised_relationships_winter.pdf", width = 10, height =7.5)
