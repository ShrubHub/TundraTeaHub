...
# Import data ----
tea <-
  read.csv(file = "scripts/users/hthomas/data/Tea/teabag_data_update.csv")
...
# pdf(file = "scripts/users/ewalker/Final_Outputs/Figures/Fig_S6.pdf",
#     width = 4,
#     height = 3)
# daily
# dev.off()
...
tea[tea$Region == "Norway", ]$Plot <-
  paste0(tea[tea$Region == "Norway", ]$Plot, "_", tea[tea$Region == "Norway", ]$Season)

# 1) Field measured variables 
# # NB - these could be updated with 2017 values
# Add plot-measured variables
Plot_Variables_Soil <-
  read.csv(file = "scripts/users/hthomas/Data/Tea/Env.Vars/Soil_Temps.csv")
Plot_Variables_Air <-
  read.csv(file = "scripts/users/hthomas/Data/Tea/Env.Vars/Air_Temps.csv")
Plot_Variables_Moisture <-
  read.csv(file = "scripts/users/hthomas/Data/Tea/Env.Vars/Moisture.csv")
...
sites_names <-
  read.csv(file = "scripts/users/hthomas/data/Tea/teabag_data_update_SiteNames_NEW.csv")
sites_names$Tea_init <-
  as.numeric(as.character(sites_names$Tea_init)) # Convert to number
sites_names$Tea_final <-
  as.numeric(as.character(sites_names$Tea_final)) # Convert to number
sites_names$Loss <-
  1 - (sites_names$Tea_final / sites_names$Tea_init) # Recalculate loss using full values
sites_names <-
  sites_names[!is.na(sites_names$Loss), ] # Remove NAs (4859 -> 4728)
sites_names <-
  sites_names[!is.na(sites_names$Days), ] # Remove NAs (none)
...
litter_Haydn <-
  read.csv("users/hthomas/tea/data/Litterbags.csv")
...
tea_CG[tea_CG$Tea_Type == "Green", ]$Tea_Type <- "green tea"
tea_CG[tea_CG$Tea_Type == "Rooibos", ]$Tea_Type <- "rooibos tea"

pdf(
  "users/hthomas/tea/figures/SF_surface.pdf",
  height = 3,
  width = 5
)
