...
# data processing ----
filenames <-
  list.files(path = "users/hthomas/tea/data/Tea/Env.Vars/Davos", pattern =
               ".{3}_.{1}_.{1}") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste("users/hthomas/tea/data/Env.Vars/Davos/", x, sep = ""),
      sep = ";",
      header = T,
      skip = 18
...
for (i in 1:length(filenames)) {
  colnames(tempdata[[i]]) <-
    as.character(unlist(tempdata[[i]][1, ]))
  tempdata[[i]] <- tempdata[[i]][-1, ]
  # put first line as header
  tempdata[[i]] <-
    tempdata[[i]][, -2]
  # delete dispensable second column
  tempdata[[i]][, 2] <- as.numeric(as.character(tempdata[[i]][, 2]))
  tempdata[[i]][, 1] <-
    as.POSIXct(tempdata[[i]][, 1], format = "%d.%m.%Y %H:%M") # convert date column into date format
  days <- tempdata[[i]][, 1]
  tempdata[[i]] <-
    aggregate(tempdata[[i]][, 2], by = list(days), mean)
  # compute mean temperatures per day
  
  fileinfo <-
    strsplit(file_path_sans_ext(filenames[i]), "_")[[1]]
  # extract plot infos from file name
  
  for (j in 1:3) {
    tempdata[[i]][, 2 + j] <- fileinfo[j]
  } # add columns for plot infos
  colnames(tempdata[[i]])[c(1:5)] <-
    c("date", "temp", "summit", "aspect", "rep")
}
...
temp <-
  subset(temp, summit == "BUF") # Only look at sites in tea bag experiment

temp$Plot <-
  paste("CH-", temp$summit, "_", temp$aspect, sep = "") # Add tea plot name
...
for (i in unique(tea[tea$Contributor == "Jonathan von Oppen, Sonja Wipf", ]$Plot)) {
  ...
  daily_temp_cut <- subset(daily_temp, Day >= DOB &
                             Day <= DOR) # Trim data
  ...
  Out <- daily_temp_cut %>%
    ...
    group_by(Plot) %>%
    summarise(
      Mean_Temperature = mean(Daily_mean),
...
# Munt Buffalora - Air
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Mt_Buff_Air/Weather_data.csv")
...
for (i in unique(tea[tea$Contributor == "Jonathan von Oppen, Sonja Wipf", ]$Plot)) {
  DOB <- min(tea[tea$Plot == i, ]$Burial) # Find date of burial
  DOR <- max(tea[tea$Plot == i, ]$Recovery) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, date >= DOB &
                             date <= DOR) # Trim data
  
  Out <- daily_temp_cut %>%
    group_by() %>%
    summarise(
      Mean_Temperature = mean(Daily_mean, na.rm = T),
...
# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/Kluane_Plateau") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Kluane_Plateau/",
        x,
        sep = ""
      ),
      sep = ",",
...
for (i in 1:length(filenames)) {
  ...
  tempdata[[i]][, 1] <-
    as.POSIXct(tempdata[[i]][, 1]) # convert date column into date format
  # days <- tempdata[[i]][,1]
  # tempdata[[i]] <- aggregate(tempdata[[i]][,2], by = list(days), mean); # compute mean temperatures per day
  
  fileinfo <-
    strsplit(file_path_sans_ext(filenames[i]), "-")[[1]]
  # extract plot infos from file name
  
  for (j in 1:4) {
    tempdata[[i]][, 3 + j] <- fileinfo[j]
  } # add columns for plot infos
  colnames(tempdata[[i]])[c(1:7)] <-
    c("date",
      "value",
      "decimal",
      "site",
...
DOR <-
  max(tea[tea$Plot == "KP_HT_winter", ]$Recovery) # Find date of recovery
daily_temp_cut <- subset(daily_temp, Day >= DOB & Day <= DOR) # Trim data
...
DOR <-
  max(tea[tea$Plot == "KP_HT_year", ]$Recovery) # Find date of recovery
daily_temp_cut <- subset(daily_temp, Day >= DOB & Day <= DOR) # Trim data
...
plot_temp_means <-
  rbind(
    plot_temp_means_summer1,
    plot_temp_means_winter,
    plot_temp_means_year,
    plot_temp_means_summer2
  )
...
plot_airtemp_means <-
  plot_temp_means[plot_temp_means$location == "Air", c(8, 2:7, 9:10)]
plot_soiltemp_means <-
  plot_temp_means[plot_temp_means$location == "Soil", c(8, 2:7, 9:10)]
...
# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Tea/Env.Vars/Pika_Camp") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Pika_Camp/",
        x,
        sep = ""
      ),
      sep = ",",
...
temp$Plot <-
  paste(substr(temp$site, 1, 2), "HT", "1yr", sep = "_") # Add tea plot name
...
# Trim data to dates of tea burial
plot_temp_means = NULL
for (i in unique(tea[tea$Site == "Pika Camp" |
                     tea$Site == "Printers Pass", ]$Plot)) {
  DOB <- min(tea[tea$Plot == i, ]$Burial) # Find date of burial
  DOR <- max(tea[tea$Plot == i, ]$Recovery) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, Day >= DOB &
                             Day <= DOR) # Trim data
...
plot_airtemp_means <-
  as.data.frame(plot_temp_means[plot_temp_means$location == "Air", -2])
plot_soiltemp_means <-
  as.data.frame(plot_temp_means[plot_temp_means$location == "Soil", -2])
...
# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/Herschel") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste("users/hthomas/tea/data/Env.Vars/Herschel/", x, sep = ""),
      sep = ",",
      header = F,
      skip = 15
    )) # read in all data files into large list
...
for (i in 1:length(filenames)) {
  ...
  colnames(tempdata[[i]])[c(1:6)] <-
    c("date", "value", "decimal", "site", "location", "rep")
}
...
# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/Abisko_2016") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Abisko_2016/",
        x,
        sep = ""
      ),
      sep = ",",
...
for (i in 1:length(filenames)) {
  ...
  colnames(tempdata[[i]])[c(1:4)] <-
    c("date", "value", "decimal", "Plot")
}
...
# Read data
temp <-
  read.table(file = "users/hthomas/tea/data/Env.Vars/Norway_2016/iButtons_Norway_2016.txt")
# Add plot and date
...
# Read data
temp <-
  read.table(file = "users/hthomas/tea/data/Env.Vars/Norway_2016/iButtons_Norway_2016AIR.txt")
# Add plot and date
temp$Plot <-
  paste(temp$mountain, temp$transect, substr(temp$plot, 1, 1), sep = "")
temp$Plot <- paste0(temp$Plot, "_Year")
...
air_plots <-
  subset(Plot_Variables_Air, Plot %notin% plot_temp_means$Plot)
...
# Read data
temp <-
  read.table(file = "users/hthomas/tea/data/Env.Vars/Abisko_2016_2/L400-900_N400-900.txt")
# Add plot and date
...
L400 <-
  subset(
    L400,
    Day != "2016-03-16 23:00:00" &
      Day != "2016-03-17 23:00:00" &
      Day != "2016-03-15 23:00:00" &
      Day != "2016-03-18 23:00:00" & Day != "2016-06-26 00:00:00"
  )
...
# Read data
temp <-
  read.table(file = "users/hthomas/tea/data/Env.Vars/Abisko_2015/iButtons_400-900masl.txt", header =
               T)
# Add plot and date
temp$Plot <- paste(temp$mountain, temp$elevation, sep = "")
temp$date <-
...
temp <-
  rbind(select(temp_2016, Plot, Day, temp), select(temp, Plot, Day, temp))
# # Plot data
# ggplot(temp,aes(date,temp,colour=factor(Plot)))+
# geom_point(size=0.1)+
# geom_smooth()+
...
# Abisko - Moisture
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Abisko_Moisture/Soil moisture_Abisko.csv")
# Add plot and date
temp$Plot <-
  paste(substr(temp$Site.name, 1, 4),
        "_",
        str_sub(temp$Site.name, start = 5),
        sep = "")
temp$date <-
  as.POSIXct(paste(temp$year, "/", temp$month, "/", temp$day, sep = ""),
             format = "%Y/%m/%d")
...
for (i in unique(tea[tea$Contributor == "Jonas Lembrechts", ]$Plot)) {
  ...
  Out <- temp %>%
    ...
    group_by(Plot) %>%
    summarise(
      Mean_moisture = mean(Humidity, na.rm = T),
...
Plot_Variables_Moisture <-
  rbind(Plot_Variables_Moisture, plot_temp_means)

# Abisko-L1000
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Abisko_Moisture/Soil moisture_L1000.csv")
# Create plot mean
...
# Norway - Moisture
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Abisko_Moisture/Soil moisture_Norway_2016.csv")
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Svalbard_other/Moisture_2016.csv")
# Add plot and date
temp$Plot <-
  paste("Svalbard", temp$Site, temp$Community, temp$Treatment, sep = "_")
temp$Date <- as.POSIXct(temp$Date, format = "%d/%m/%Y")
temp$Date <- temp$Date + years(2000) # correct years
...
# Find missing plots
missing <-
  unique(subset(tea[tea$Contributor == "Matteo Bon Petit", ], Plot %notin% plot_moisture_means$Plot)$Plot)
plot_moisture_means_missing <-
  plot_moisture_means[c(1:length(missing)), ]
plot_moisture_means_missing[, c(2:5)] <- NA
plot_moisture_means_missing$Plot <- as.character(missing)

# Add higher level plot
plot_moisture_means_missing$Site <-
  substr(plot_moisture_means_missing$Plot, 1, 13)
...
plot_moisture_means_missing$Mean_moisture <-
  PMM_averages$Mean_moisture[match(plot_moisture_means_missing$Site, PMM_averages$Site)]
plot_moisture_means_missing$sd_moisture <-
  PMM_averages$sd_moisture[match(plot_moisture_means_missing$Site, PMM_averages$Site)]
plot_moisture_means_missing$source <-
  "estimated from surrounding plots"
plot_moisture_means_missing$scale <- "Site"

plot_moisture_means <-
  rbind(plot_moisture_means[, -6], plot_moisture_means_missing[, -6])

Svalbard_sites <- plot_moisture_means

Plot_Variables_Moisture <-
  rbind(Plot_Variables_Moisture, plot_moisture_means)

# Svalbard - Soil Temperature

# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/Svalbard_Temp") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Svalbard_Temp/",
        x,
        sep = ""
      ),
      sep = ",",
...
temp$Plot <-
  paste("Svalbard", temp$site, temp$community, temp$treatment, sep = "_") # Add tea plot name
...
plot_soiltemp_means <-
  as.data.frame(plot_temp_means[plot_temp_means$location == "D", -2])
# Label
plot_soiltemp_means$source <- "ibutton data (6cm depth)"
plot_soiltemp_means$scale <- "plot"
# Pull out shallow
plot_soil_shallow_means <- NULL
plot_soil_shallow_means <-
  as.data.frame(plot_temp_means[plot_temp_means$location == "S", -2])
plot_soil_shallow_means <-
  subset(plot_soil_shallow_means,
         Plot %notin% plot_soiltemp_means$Plot)
# Label
...
# Combine
plot_soiltemp_means <-
  rbind(plot_soiltemp_means, plot_soil_shallow_means)

# Identify  missing sites
# Find missing plots
missing <-
  unique(subset(tea[tea$Contributor == "Matteo Bon Petit", ], Plot %notin% plot_soiltemp_means$Plot)$Plot)
plot_soiltemp_means_missing <-
  plot_soiltemp_means[c(1:length(missing)), ]
plot_soiltemp_means_missing[, c(2:5)] <- NA
plot_soiltemp_means_missing$Plot <- as.character(missing)

# Add higher level plot
plot_soiltemp_means_missing$Site <-
  substr(plot_soiltemp_means_missing$Plot, 1, 13)
...
# Add to missing plots
plot_soiltemp_means_missing$Mean_Temperature <-
  PTM_averages$Mean_Temperature[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$sd_Temperature <-
  PTM_averages$sd_Temperature[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$GSL_0 <-
  PTM_averages$GSL_0[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$GSL_5 <-
  PTM_averages$GSL_5[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$GDD_0 <-
  PTM_averages$GDD_0[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$GDD_5 <-
  PTM_averages$GDD_5[match(plot_soiltemp_means_missing$Site, PTM_averages$Site)]
plot_soiltemp_means_missing$source <-
  "estimated from surrounding plots"
plot_soiltemp_means_missing$scale <- "Site"

plot_soiltemp_means <-
  rbind(plot_soiltemp_means[, -10], plot_soiltemp_means_missing[, -10])
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Svalbard_other/Weather_station.csv",
           header = F,
           skip = 1)
...
filenames <-
  list.files("users/hthomas/tea/data/Env.Vars/Fairbanks/")
sites <-
  c(
    "Cold-Forest",
    "Cold-Treeline",
...
for (i in 1:length(filenames))
{
  d[[i]] <-
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Fairbanks/",
        filenames[i],
        sep = ""
      ),
      sep = ",",
...
plot_soiltemp_means <-
  subset(plot_temp_means, location == "-0.2m" | location == "0m")
...
plot_soiltemp_means$source <-
  "ibutton data (mean of 20cm deep and surface(0cm))"
plot_soiltemp_means$scale <- "plot"

Plot_Variables_Soil <- rbind(Plot_Variables_Soil, plot_soiltemp_means)

plot_airtemp_means <-
  subset(plot_temp_means, location == "2m" | location == "0.5m")
...
# Pentlands
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Pentlands/Pentlands.temp.csv")
...
# Gavia
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Gavia/gavia_ita_env_data_2016&2017_v2.csv", header =
             TRUE)

# Add plot and date
temp$Plot <-
  paste("GV",
        substr(temp$community, 1, 1),
        substr(temp$treatment, 1, 1),
        sep = "_")

temp[temp$Incubation == "Winter", ]$Plot <-
  paste0(temp[temp$Incubation == "Winter", ]$Plot, "_Winter")
temp[temp$Incubation == "year", ]$Plot <-
  paste0(temp[temp$Incubation == "year", ]$Plot, "_Year")
...
# Read Data
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Quebec/Quebec.csv")
# Raw Data
...
for (i in unique(tea[tea$Contributor == "Sandra Angers-Blondin, Stephane Boudreau", ]$Plot)) {
  ...
  Out <- daily_temp_cut %>%
    summarise(
      Mean_Temperature = mean(Daily_mean, na.rm = TRUE),
...
# Lofoten
temp_w <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Lofoten/Leknes_W.csv")
temp_w$station <- "W"
temp_l <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Lofoten/Leknes_L.csv")
temp_l$station <- "L"
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Greenland_Toke/Soil_moisture.csv")

# Add plot and date
temp$date <-
  as.POSIXct(paste(temp$Day, temp$Month, temp$Year, sep = "/"), format = "%d/%m/%Y")
temp$Plot <-
  ifelse(
    temp$Site == "N",
    paste(
      "Narsarsuaq",
      paste(temp$Elevation, temp$Habitat, temp$Plot, sep = ""),
      temp$Treatment,
      sep = "_"
    ),
    paste(
      "Qaqortoq",
      paste("Q", temp$Elevation, temp$Habitat, temp$Plot, sep = ""),
      temp$Treatment,
      sep = "_"
    )
  )
...
daily_temp <- temp %>%
  group_by(Plot, date) %>%
  summarise(
    Daily_mean = mean(Soilmoist, na.rm = TRUE),
...
# Select plots
plots <-
  subset(daily_temp, Plot %in% tea[tea$Contributor == "Toke Thomas Hoye" &
                                     tea$Site != "Greenland", ]$Plot)$Plot
for (i in unique(plots)) {
  DOB <- min(tea[tea$Plot == i, ]$Burial, na.rm = T) # Find date of burial
  DOR <-
    max(tea[tea$Plot == i, ]$Recovery, na.rm = T) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, date >= DOB &
                             date <= DOR) # Trim data
...
# Add missing sites
missing <-
  unique(subset(tea[tea$Contributor == "Toke Thomas Hoye" &
                      tea$Site != "Greenland", ], Plot %notin% Plot_Variables_Moisture$Plot)$Plot)
...
for (i in unique(plots)) {
  ...
  Out <- daily_temp_cut %>%
    filter(Plot == i) %>%
    summarise(
      Mean_Temperature = mean(Daily_mean_temp, na.rm = T),
...
missing <-
  unique(subset(tea[tea$Contributor == "Toke Thomas Hoye" &
                      tea$Site != "Greenland", ], Plot %notin% Plot_Variables_Soil$Plot)$Plot)
plot_temp_means <- plot_temp_means[c(1:length(missing)), ]
...
# Greenland (Toke - Air)
temp_N <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Greenland_Toke/N.csv")
temp_N$Site <- "Narsarsuaq"
temp_Q <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Greenland_Toke/Q.csv")
...
# Trim data to dates of tea burial
plot_temp_means = NULL
for (i in unique(tea[tea$Contributor == "Toke Thomas Hoye" &
                     tea$Site != "Greenland" & !is.na(tea$Burial), ]$Plot)) {
  DOB <- min(tea[tea$Plot == i, ]$Burial, na.rm = T) # Find date of burial
  DOR <-
    max(tea[tea$Plot == i, ]$Recovery, na.rm = T) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, date >= DOB &
...
# Iceland
temp_e <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Iceland/E.csv")
temp_e$station <- "E"
temp_N <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Iceland/N.csv")
temp_N$station <- "N"
temp_w <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Iceland/W.csv")
temp_w$station <- "W"
...
temp_l <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Iceland/L.csv")
temp_l$station <- "L"
temp_l$Site <- "Endalen"
names(temp_l)[1] <- "GMT"
...
temp <-
  select(temp, Date = date, Air_Temp = Mean.TemperatureC, Day, station, Site)
...
for (i in unique(tea[tea$Contributor == "Isabel Barrio, Ingibj\x9arg Svala J\x97nsd\x97ttir, Katr\x92n Bj\x9arnsd\x97ttir", ]$Plot)) {
  ...
  j <-
    ifelse(
      substr(i, 1, 1) == "A",
      "Auokiluheioi",
      ifelse(substr(i, 1, 1) == "P", "Theistareykir", "Endalen")
    )
...
# Latnja
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Latnja/Temp.station.csv")
...
temp <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/CG/Kluane_-_Common_garden_3Jun2016_dateastext.csv"
  )
...
temp2 <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/CG/Kluane_-_Common_garden_3Jun2016_dateasdate.csv"
  )

temp2$date <-
  as.POSIXct(temp2$Date.Time..GMT.06.00, format = "%m/%d/%Y %H:%M")
...
plot_temp_means = NULL
for (i in unique(tea[tea$Contributor == "Eleanor Walker, Haydn Thomas" &
                     tea$Site != "Common Garden", ]$Plot)) {
  DOB <-
    as.numeric(strftime(min(tea[tea$Plot == "10A", ]$Burial), format = "%j")) # Find date of burial
  DOR <-
    as.numeric(strftime(max(tea[tea$Plot == "10A", ]$Recovery), format = "%j")) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, doy >= DOB &
                             doy <= DOR) # Trim data
...
for (i in unique(plot_temp_means$Plot)) {
  ...
  Mean_Temperature <-
    unique(
      plot_temp_means[plot_temp_means$Site == "KP_HT_s2" &
                        plot_temp_means$Plot == i, ]$Mean_Temperature * multKP + plot_temp_means[plot_temp_means$Site ==
                                                                                                   "CG_HT_year" & plot_temp_means$Plot == i, ]$Mean_Temperature * multCG
    )
  GDD_0 <-
    unique(plot_temp_means[plot_temp_means$Site == "KP_HT_s2" &
                             plot_temp_means$Plot == i, ]$GDD_0 * multKP + plot_temp_means[plot_temp_means$Site ==
                                                                                             "CG_HT_year" & plot_temp_means$Plot == i, ]$GDD_0 * multCG)
  GDD_5 <-
    unique(plot_temp_means[plot_temp_means$Site == "KP_HT_s2" &
                             plot_temp_means$Plot == i, ]$GDD_5 * multKP + plot_temp_means[plot_temp_means$Site ==
                                                                                             "CG_HT_year" & plot_temp_means$Plot == i, ]$GDD_5 * multCG)
  GSL_0 <-
    unique(plot_temp_means[plot_temp_means$Site == "KP_HT_s2" &
                             plot_temp_means$Plot == i, ]$GSL_0 * multKP + plot_temp_means[plot_temp_means$Site ==
                                                                                             "CG_HT_year" & plot_temp_means$Plot == i, ]$GSL_0 * multCG)
  GSL_5 <-
    unique(plot_temp_means[plot_temp_means$Site == "KP_HT_s2" &
                             plot_temp_means$Plot == i, ]$GSL_5 * multKP + plot_temp_means[plot_temp_means$Site ==
                                                                                             "CG_HT_year" & plot_temp_means$Plot == i, ]$GSL_5 * multCG)
  
  plot_temp_means[plot_temp_means$Plot == i, ]$Mean_Temperature <-
    Mean_Temperature
...
# Read Data
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/Kluane_Gradient/") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/Kluane_Gradient/",
        x,
        sep = ""
      ),
      sep = ",",
...
for (i in unique(daily_temp$Plot)) {
  ...
  Out <- daily_temp_cut %>%
    ...
    group_by(Plot) %>%
    summarise(
      Mean_Temperature = mean(Daily_mean_soil),
...
# Add missing sites
missing <-
  unique(subset(tea[tea$Contributor == "Eleanor Walker, Haydn Thomas" &
                      tea$Site != "Common Garden", ], Plot %notin% Plot_Variables_Soil$Plot)$Plot)
plot_temp_means <- plot_temp_means[c(1:length(missing)), ]
plot_temp_means[, c(2:5)] <- NA
plot_temp_means$Plot <- missing
Plot_Variables_Soil <- rbind(Plot_Variables_Soil, plot_temp_means)
...
# CG Tea
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/CG/ibuttons/") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste(
        "users/hthomas/tea/data/Env.Vars/CG/ibuttons/",
        x,
        sep = ""
      ),
      sep = ",",
...
for (i in 1:length(filenames)) {
  ...
  colnames(tempdata[[i]])[c(1:5)] <-
    c("date", "value", "decimal", "location", "replicate")
}
...
temp$source <- "ibutton"
temp$value <-
  as.numeric(as.character(paste(temp$value, temp$decimal, sep = ".")))
temp <- temp[, -3]
...
CG_temp[CG_temp$location == "Moisture" & CG_temp$value < 0, ]$value <-
  NA
...
# Trim data to dates of tea burial
plot_temp_means = NULL
for (i in unique(tea[tea$Site == "Common Garden" &
                     substr(tea$Plot, 1, 5) != "CG_DT" &
                     tea$Plot != "DC_HT_1yr" & tea$Contributor == "Haydn Thomas", ]$Plot)) {
  DOB <- min(tea[tea$Plot == i, ]$Burial) # Find date of burial
  DOR <- max(tea[tea$Plot == i, ]$Recovery) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, Day >= DOB &
                             Day <= DOR) # Trim data
...
plot_soiltemp_means <-
  subset(plot_temp_means, location == "Soil" & source == "ibutton")[, -1]
Plot_Variables_Soil <- rbind(Plot_Variables_Soil, plot_soiltemp_means)

plot_soiltemp_means <-
  subset(plot_temp_means,
         location == "Soil" & source == "HOBO" & Plot == "CG_Y2_3m")[, -1]
Plot_Variables_Soil <- rbind(Plot_Variables_Soil, plot_soiltemp_means)

plot_airtemp_means <-
  subset(plot_temp_means, location == "Air" & source == "ibutton")[, -1]
Plot_Variables_Air <- rbind(Plot_Variables_Air, plot_airtemp_means)

plot_airtemp_means <-
  subset(plot_temp_means,
         location == "Air" & source == "HOBO" & Plot == "CG_Y2_3m")[, -1]
Plot_Variables_Air <- rbind(Plot_Variables_Air, plot_airtemp_means)

plot_moisture_means <-
  subset(plot_temp_means, location == "Moisture")[, -c(1, 5:8)]
plot_moisture_means <-
  filter(plot_moisture_means,
         Plot == "CG_HT_year" | Plot == "CG_HT_winter")
...
# El Experiment - Soil
filenames <-
  list.files(path = "users/hthomas/tea/data/Env.Vars/CG_Expmt/") # create list of temperature data files

tempdata <-
  lapply(filenames, function(x)
    read.csv(
      paste("users/hthomas/tea/data/Env.Vars/CG_Expmt/", x, sep = ""),
      sep = ",",
      header = F,
      skip = 15
    )) # read in all data files into large list
...
for (i in 1:length(filenames)) {
  ...
  # tempdata[[i]][is.na(tempdata[[i]]$Decimal),3] <- 0
  tempdata[[i]][, 1] <-
    as.POSIXct(tempdata[[i]][, 1], format = "%d/%m/%y %H:%M:%S") # convert date column into date format
  # days <- tempdata[[i]][,1]
  # tempdata[[i]] <- aggregate(tempdata[[i]][,2], by = list(days), mean); # compute mean temperatures per day
...
plot_moisture_means <-
  subset(plot_temp_means, location == "Moisture")[, -c(1, 4:7)]
...
temp <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/Kluane_Moisture/Plat_Moisture_Data.csv"
  )
...
temp <-
  read.csv("users/hthomas/tea/data/Env.Vars/Kluane_Moisture/CG_Soil_Moisture.csv")
...
Plot_Variables_Moisture <-
  rbind(Plot_Variables_Moisture, plot_temp_means[-1])

# Add ambient moisture to daily tea and common garden
plot_temp_means_ambient <-
  plot_temp_means[plot_temp_means$Plot_Type == "CTL", ][1, ]
...
plot_temp_means <-
  rbind(plot_temp_means,
        cbind(Plot = "CG_Y2_3m", plot_temp_means_ambient)[, c(3, 4, 1, 6)])


# Trail Creek
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Inuvik/TM2_2016_EditedforHaydn_Jan-20-17_CW.csv")
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Inuvik_Moisture/Env.Variable.Extraction.csv")
...
plot_moisture_means <- temp %>%
  group_by(Plot) %>%
  summarise(
    Mean_moisture = mean(Fivecm, na.rm = T),
...
# Kangerlussuaq
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Kangerlussuaq/Town.csv")
...
temp2 <-
  read.table(file = "users/hthomas/tea/data/Env.Vars/Kangerlussuaq/KAN_B_hour.txt", header =
               T)

# Add plot and date
temp2$date <-
  as.POSIXct(paste(temp2$Year, "/", temp2$MonthOfYear, "/", temp2$DayOfMonth, sep =
                     ""),
             format = "%Y/%m/%d")
...
# Toolik
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Toolik/2011-Present_MAT_HourlyWeather.csv")
...
temp2 <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Toolik/1999-present_MNT_air_temperature.csv")
...
for (i in unique(tea[tea$Contributor == "Laura Gough, Dan Ackerman", ]$Plot)) {
  DOB <-
    as.numeric(strftime(min(tea[tea$Plot == i, ]$Burial), format = "%j")) # Find date of burial
  DOR <-
    as.numeric(strftime(max(tea[tea$Plot == i, ]$Recovery), format = "%j")) # Find date of recovery
  daily_temp_cut <- subset(daily_temp, doy >= DOB &
                             doy <= DOR) # Trim data
  daily_temp_cut <-
    rbind(daily_temp, daily_temp_cut) # Add back in as full year
...
# Gothic
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_air_temperature/Gothic_aspen_forest_air_temperature_2016.csv")
temp <-
  rbind(
    temp,
    read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_air_temperature/Gothic_rocky_meadow_air_temperature_2016.csv")
  )
temp <-
  rbind(
    temp,
    read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_air_temperature/Gothic_veratrum_meadow_air_temperature_2016.csv")
  )
temp <-
  rbind(
    temp,
    read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_air_temperature/Gothic_wet_meadow_air_temperature_2016.csv")
  )
temp <-
  rbind(
    temp,
    read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_air_temperature/Gothic_willow-wet_meadow_interface_air_temperature_2016.csv")
  )
...
daily_temp[daily_temp$habitat == "willow-wet.meadow.interface", ]$habitat <-
  "Willow-wet.meadow.interface"
daily_temp[daily_temp$habitat == "rocky.meadow", ]$habitat <-
  "Rocky.meadow"
daily_temp[daily_temp$habitat == "aspen.forest", ]$habitat <-
  "Aspen.forest"
daily_temp[daily_temp$habitat == "veratrum.meadow", ]$habitat <-
  "Veratrum.meadow"
daily_temp[daily_temp$habitat == "wet.meadow", ]$habitat <-
  "Wet.meadow"
...
for (i in unique(tea[tea$Contributor == "David Inouye, Jane Ogilvie", ]$Plot)) {
  ...
  DOR <- max(tea[tea$Plot == i, ]$Recovery) # Find date of recovery
  daily_temp_cut <-
    subset(daily_temp, Date >= DOB & Date <= DOR &
             habitat == i) # Trim data
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_soil_moisture/Gothic_rocky_meadow_soil_moisture_2016.csv")
temp <-
  rbind(
    temp,
    read.csv(file = "users/hthomas/tea/data/Env.Vars/Gothic/Habitat_soil_moisture/Gothic_wet_meadow_soil_moisture_2016.csv")
  )
...
# Take mean moisture
temp$moisture <-
  rowMeans(temp[c('south_5cm', 'north_5cm')], na.rm = TRUE) * 100
...
for (i in unique(tea[tea$Contributor == "David Inouye, Jane Ogilvie", ]$Plot)) {
  ...
  Out <- daily_temp_cut %>%
    ungroup() %>%
    summarise(
      Mean_moisture = mean(Daily_mean, na.rm = T),
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/ValBercla/2014_temps.csv")
...
# Trim data to dates of tea burial
plot_temp_means = NULL
for (i in unique(tea[tea$Site == "Val Bercla" &
                     tea$Season == "Summer", ]$Plot)) {
  DOB <-
    as.numeric(strftime(min(tea[tea$Plot == "10A", ]$Burial), format = "%j")) # Find date of burial
  DOR <-
    as.numeric(strftime(max(tea[tea$Plot == "10A", ]$Recovery), format = "%j")) # Find date of recovery
...
temp <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/ValBercla/valbercla_near6_winter2015.csv",
    skip = 19
  ) # read in all data files into large list

temp$date <-
  as.POSIXct(temp$Date.Time, format = "%d.%m.%Y %H:%M") # convert date column into date format
temp <- temp[, -c(1, 2)]
temp$Day <- cut(temp$date, "day") # Extract days
...
# Val Bercla - Air
temp <-
  read.csv("users/hthomas/tea/data/Env.Vars/ValBercla/weather_station.csv")
...
# Stillberg - Air
temp <-
  read.csv("users/hthomas/tea/data/Env.Vars/Stillberg/weather_station.csv")
...
# Japan
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Japan/soil_temps.csv")
...
for (i in unique(tea[tea$Contributor == "Satoshi Suzuki", ]$Plot)) {
  Mean_Temperature <-
    subset(temp, Plot == i)$Soil.temperature.during.experimental.period
  sd_Temperature = NA
  GSL_0 = NA
  GSL_5 = NA
  GDD_0 = NA
...
# Southampton Island
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Southampton_Island/weather_data.csv")
...
temp <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/QHI_Phenology/QHI_Phenology_asText.csv"
  )
...
temp2 <-
  read.csv(
    "users/hthomas/tea/data/Env.Vars/QHI_Phenology/QHI_Phenology_asDate.csv"
  )
...
# QHI (Isla) - Moisture
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/QHI_Moisture/QHI_Moisture.csv")
...
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Disko_Island/Soil_temp_moisture.csv")
temp <- melt(
  temp,
  variable.name = "Plot",
  value.names = "value",
...
# Moisture
plot_moisture_means <-
  subset(temp, Site_name == "Soil_moisture")[, c(2, 3)]
...
# Air Temp
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Disko_Island/air_temp.csv")
temp$Date <- as.POSIXct(temp$Date, format = "%d/%m/%y")
...
# Abisko - air temp
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Abisko_Air/Air_Temps.csv")
...
tea.abisko <- tea %>%
  filter(Region == "Abisko" |
           Region == "Abisko 2" | Region == "Latnja")
tea.abisko <- as.data.frame(unique(tea.abisko$Plot))
tea.abisko$Air_Temp <-
  Plot_Variables_Air$Mean_Temperature[match(tea.abisko$`unique(tea.abisko$Plot)`,
                                            Plot_Variables_Air$Plot)]
...
for (i in unique(missing.abisko.air$Plot)) {
  ...
  Out <- daily_temp_cut %>%
    summarise(
      Mean_Temperature = mean(AirTemperature, na.rm = T),
...
# AIR# 
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Finland/air_temp_logger.csv")
...
# SOIL# 
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Finland/soil_temp_logger.csv")
...
for (i in unique(tea[tea$Region == "Finland", ]$Plot)) {
  temp_cut <- subset(temp, Plot == i)
  DOB <-
    min(tea[tea$Plot == i, ]$Burial) + years(2000) # Find date of burial
  DOR <-
    max(tea[tea$Plot == i, ]$Recovery) + years(2000) # Find date of recovery
  daily_temp_cut <- subset(temp_cut, Day >= DOB & Day <= DOR) # Trim data
...
# Soil moisture
temp <-
  read.csv(file = "users/hthomas/tea/data/Env.Vars/Finland/metadata_v3.csv")

temp_moisture <-
  select(temp, site, soil_moisture_mean, soil_moisture_stdev)
# Winter
...
plot_moisture_means <-
  select(plot_moisture_means,
         Plot,
         Mean_moisture = soil_moisture_mean,
         sd_moisture = soil_moisture_stdev)
plot_moisture_means$source <- "Moisture Probe"
...
weather_station_meta <-
  read.csv("users/hthomas/tea/data/Env.Vars/Weather_Elevations.csv")

correction_plots <-
  select(subset(tea, Site %in% weather_station_meta$Site),
         Site,
         Plot,
         Elevation)
correction_plots$Site_ele <-
  as.numeric(as.character(weather_station_meta$Elevation[match(correction_plots$Site, weather_station_meta$Site)]))
correction_plots$Diff <-
  correction_plots$Site_ele - as.numeric(as.character(correction_plots$Elevation))
correction_plots$diff_temp <- correction_plots$Diff * 0.005

correct <- subset(Plot_Variables_Air, Plot %in% correction_plots$Plot)
Plot_Variables_Air <-
  subset(Plot_Variables_Air, Plot %notin% correction_plots$Plot)

correct$diff_temp <-
  correction_plots$diff_temp[match(correct$Plot, correction_plots$Plot)]
...
# Add variables
checking$Air <-
  Plot_Variables_Air$Mean_Temperature[match(checking$Plot, Plot_Variables_Air$Plot)]
checking$Soil <-
  Plot_Variables_Soil$Mean_Temperature[match(checking$Plot, Plot_Variables_Soil$Plot)]
checking$Moisture <-
  Plot_Variables_Moisture$Mean_moisture[match(checking$Plot, Plot_Variables_Air$Plot)]
...
# Final export script
write.csv(Plot_Variables_Soil, file = "users/hthomas/tea/data/Env.Vars/Soil_Temps.csv")
write.csv(Plot_Variables_Air, file = "users/hthomas/tea/data/Env.Vars/Air_Temps.csv")
write.csv(Plot_Variables_Moisture, file = "users/hthomas/tea/data/Env.Vars/Moisture.csv")
