...
(
  air <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(airtemp_mean + airtemp_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, factor = 0.05),
    ...
    labs(x = "Site summer air temp. (°C)", y = "Mass loss (%, summer)") +
...
(
  soil <- ggplot() +
    geom_point(
      data = season_narm_r_soil,
      aes(
        x = jitter(soiltemp_mean + soiltemp_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
      alpha = 0.3
    ...
    labs(x = "Site summer soil temp. (°C)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
(
  moisture <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(moisture_mean + moisture_cent_amount, amount = 2.5),
        y = jitter(Loss * 100, amount = 0.05),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    labs(x = "Site summer soil moist. (%)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
predsout.space_CHELSA_summer_temp$CHELSA_summer_temp <-
  rep(jags.dat_CHELSA_summer_temp$xhat1, each = 2)
predsout.space_CHELSA_summer_temp$Temp <-
  rep(jags.dat_CHELSA_summer_temp$xhat2, each = 2)
predsout.space_CHELSA_summer_temp$Tea_TypeNum <-
  rep(c(1, 2), times = (length(predsout.space_CHELSA_summer_temp$mean) /
                          2))
predsout.space_CHELSA_summer_temp$Tea_Type <-
  ifelse(predsout.space_CHELSA_summer_temp$Tea_TypeNum == 1,
         "Green",
         "Rooibos")
...
(
  CHELSA_summer_temp <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(CHELSA_summer_temp + CHELSA_summer_temp_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    labs(x = "Gridded summer air temp (°C)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
predsout.space_CHELSA_summer_precip$CHELSA_summer_precip <-
  rep(jags.dat_CHELSA_summer_precip$xhat1, each = 2)
predsout.space_CHELSA_summer_precip$Temp <-
  rep(jags.dat_CHELSA_summer_precip$xhat2, each = 2)
predsout.space_CHELSA_summer_precip$Tea_TypeNum <-
  rep(c(1, 2), times = (length(
    predsout.space_CHELSA_summer_precip$mean
  ) / 2))
predsout.space_CHELSA_summer_precip$Tea_Type <-
  ifelse(predsout.space_CHELSA_summer_precip$Tea_TypeNum == 1,
         "Green",
         "Rooibos")
...
(
  CHELSA_summer_precip <- ggplot() +
    ...
    labs(x = "Gridded summer precip. (mm)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
(
  ESA_moisture <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(ESA_moisture + ESA_moisture_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    labs(x = "Gridded summer soil moist. (%)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
season_narm_r$envlevel_moisture <-
  ifelse(
    season_narm_r$ESA_cell %in% env.levels2$ESA_cell[env.levels2$n.plots > 1],
    1,
    season_narm_r$envlevel_moisture
  )
...
season_narm_r$envlevel <-
  apply(season_narm_r[, 64:65], 1, max)# Add categories
...
# AB: REMOVE MISSING VALUES OF SOIL airtemp AND TEMPERATURE FOR THE airtemp X TEMPERATURE INTERACTION MODEL
season_narm_r <-
  season_narm_r[!is.na(season_narm_r$airtemp_mean) &
                  !is.na(season_narm_r$moisture_mean), ]
...
season_narm_r$moisture_mean <-
  scale(season_narm_r$moisture_mean,
        center = TRUE,
        scale = FALSE)
days_cent_amount_air <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
       'scaled:center')
days_scale_amount_air <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
       'scaled:scale')
season_narm_r$Days <-
  scale(season_narm_r$Days, center = TRUE, scale = FALSE)
...
season_narm_r$airtemp_sd_site <-
  season_narm_r_sites$airtemp_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$moist_mean_site <-
  season_narm_r_sites$moist_mean_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$moist_sd_site <-
  season_narm_r_sites$moist_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]

season_narm_r$airtemp_sd_site[season_narm_r$airtemp_sd_site == 0] <-
  mean(season_narm_r$airtemp_sd_site[season_narm_r$airtemp_sd_site > 0], na.rm = T)
season_narm_r$airtemp_sd_site[is.na(season_narm_r$airtemp_sd_site)] <-
  0.001
season_narm_r$moist_sd_site[season_narm_r$moist_sd_site == 0] <-
  mean(season_narm_r$moist_sd_site[season_narm_r$moist_sd_site > 0], na.rm = T)
season_narm_r$moist_sd_site[is.na(season_narm_r$moist_sd_site)] <-
  0.001
...
season_narm_r$airtemp_sd_region <-
  season_narm_r_regions$airtemp_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$moist_mean_region <-
  season_narm_r_regions$moist_mean_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$moist_sd_region <-
  season_narm_r_regions$moist_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
...
season_narm_r$airtemp_sd_region[is.na(season_narm_r$airtemp_sd_region)] <-
  0.001
season_narm_r$moist_sd_region[season_narm_r$moist_sd_region == 0] <-
  mean(season_narm_r$moist_sd_region[season_narm_r$moist_sd_region > 0], na.rm = T)
season_narm_r$moist_sd_region[is.na(season_narm_r$moist_sd_region)] <-
  0.001
...
moist_x_cent_amount_air <-
  attr(scale(
    c(LQ_moist, UQ_moist, mean_moist, med_moist),
    center = TRUE,
    scale = FALSE
  ), 'scaled:center')
moist_x_scale_amount_air <-
  attr(scale(
    c(LQ_moist, UQ_moist, mean_moist, med_moist),
    center = TRUE,
    scale = FALSE
...
xhats <-
  expand.grid(
    xhat1 = seq(min_airtemp, max_airtemp, by = 0.01),
    xhat2 = scale(
      c(LQ_moist, UQ_moist, mean_moist, med_moist),
      center = TRUE,
      scale = FALSE
    ...
    xhat3 = mean_burial
  ) # AB: predicting soil airtemp at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
load("users/hthomas/tea/Stan_outputs/airtemp_moisture_fits_tea_int_unscaled.Rdata")

predsout.space_air_moisture <- cout[cout$Param %in% c("preds"), ]
predsout.space_air_moisture$airtemp <-
  rep(jags.dat_air_interaction$xhat1, each = 2)
predsout.space_air_moisture$TempBT <-
  predsout.space_air_moisture$airtemp  + airtemp_cent_amount_air
predsout.space_air_moisture$Moisture <-
  rep(jags.dat_air_interaction$xhat2, each = 2)
predsout.space_air_moisture$MoistureBT <-
  predsout.space_air_moisture$Moisture + moist_x_cent_amount_air
predsout.space_air_moisture$Tea_TypeNum <-
  rep(c(1, 2), times = (length(predsout.space_air_moisture$mean) / 2))
predsout.space_air_moisture$Tea_Type <-
  ifelse(predsout.space_air_moisture$Tea_TypeNum == 1,
         "Green",
         "Rooibos")
...
(
  air_moisture <- ggplot() +
    geom_point(
      data = season_narm_r_air_moisture[season_narm_r_air_moisture$Tea_Type ==
                                          "Green", ],
      aes(
        x = jitter(airtemp_mean + airtemp_cent_amount_air, amount = 0.25),
        y = jitter(Loss, amount = 0.05)
      ),
      colour = "#006400",
      pch = 16 ,
      alpha = 0.3
    ...
    geom_ribbon(
      data = predsout.space_air_moisture[predsout.space_air_moisture$MoistureBT ==
                                           LQ_moist |
                                           predsout.space_air_moisture$MoistureBT == UQ_moist, ],
      aes(
        x = airtemp + airtemp_cent_amount_air,
        ymin = (`2.5%`),
        ymax = (`97.5%`),
        fill = factor(Tea_Type):factor(Moisture)
      ),
      alpha = 0.5
    ) +
    geom_line(
      ...
      aes(
        x = airtemp + airtemp_cent_amount_air,
        y = mean,
        colour = factor(Tea_Type):factor(Moisture)
      ),
      alpha = 0.8,
      lwd = 1.5
    ) +
    ...
    scale_colour_manual(
      values = c("#00b100", "#003100", "#c83232", "#4e1414"),
      name = "Tea Type"
    ) +
    scale_fill_manual(
      values = c("#00b100", "#003100", "#c83232", "#4e1414"),
...
season_narm_r$moisture_mean <-
  scale(season_narm_r$moisture_mean,
        center = TRUE,
        scale = FALSE)
days_cent_amount_soil <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
       'scaled:center')
days_scale_amount_soil <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
       'scaled:scale')
season_narm_r$Days <-
  scale(season_narm_r$Days, center = TRUE, scale = FALSE)
...
moist_x_cent_amount_soil <-
  attr(scale(
    c(LQ_moist, UQ_moist, mean_moist, med_moist),
    center = TRUE,
    scale = FALSE
  ), 'scaled:center')
moist_x_scale_amount_soil <-
  attr(scale(
    c(LQ_moist, UQ_moist, mean_moist, med_moist),
    center = TRUE,
    scale = FALSE
...
xhats <-
  expand.grid(
    xhat1 = seq(min_soiltemp, max_soiltemp, by = 0.01),
    xhat2 = scale(
      c(LQ_moist, UQ_moist, mean_moist, med_moist),
      center = TRUE,
      scale = FALSE
    ...
    xhat3 = mean_burial
  ) # AB: predicting soil soiltemp at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
predsout.space_soil_moisture$soiltemp <-
  rep(jags.dat_soil_interaction$xhat1, each = 2)
predsout.space_soil_moisture$TempBT <-
  predsout.space_soil_moisture$soiltemp  + soiltemp_cent_amount_soil
predsout.space_soil_moisture$Moisture <-
  rep(jags.dat_soil_interaction$xhat2, each = 2)
predsout.space_soil_moisture$MoistureBT <-
  predsout.space_soil_moisture$Moisture + moist_x_cent_amount_soil
predsout.space_soil_moisture$Tea_TypeNum <-
  rep(c(1, 2), times = (length(predsout.space_soil_moisture$mean) / 2))
predsout.space_soil_moisture$Tea_Type <-
  ifelse(predsout.space_soil_moisture$Tea_TypeNum == 1,
         "Green",
         "Rooibos")
...
(
  soil_moisture <- ggplot() +
    geom_point(
      data = season_narm_r_soil_moisture[season_narm_r_soil_moisture$Tea_Type ==
                                           "Green", ],
      aes(
        x = jitter(soiltemp_mean + soiltemp_cent_amount_soil, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05)
      ),
      colour = "#15BB15",
      pch = 16 ,
    ...
    geom_ribbon(
      data = predsout.space_soil_moisture[predsout.space_soil_moisture$MoistureBT ==
                                            LQ_moist |
                                            predsout.space_soil_moisture$MoistureBT == UQ_moist, ],
      aes(
        x = soiltemp + soiltemp_cent_amount_soil,
        ymin = (`2.5%` * 100),
        ymax = (`97.5%` * 100),
        fill = factor(Tea_Type):factor(Moisture)
      ),
    ...
    coord_cartesian(y = c(0, 100)) +
    scale_colour_manual(
      values = c("#15BB15", "#003100", "#9A0C0C", "#4e1414"),
      name = "Tea Type"
    ) +
    scale_fill_manual(
      values = c("#15BB15", "#003100", "#9A0C0C", "#4e1414"),
...
season_narm_r <-
  season_narm_r[!is.na(season_narm_r$CHELSA_summer_temp) &
                  !is.na(season_narm_r$ESA_moisture), ]
...
season_narm_r$CHELSA_summer_temp_sd_site[season_narm_r$CHELSA_summer_temp_sd_site ==
                                           0 |
                                           is.na(season_narm_r$CHELSA_summer_temp_sd_site)] <-
  mean(season_narm_r$CHELSA_summer_temp_sd_site[season_narm_r$CHELSA_summer_temp_sd_site >
                                                  0], na.rm = T)
season_narm_r$ESA_moisture_sd_site[season_narm_r$ESA_moisture_sd_site ==
                                     0 |
                                     is.na(season_narm_r$ESA_moisture_sd_site)] <-
  0.01
...
season_narm_r$ESA_moisture_sd_region[season_narm_r$ESA_moisture_sd_region ==
                                       0 |
                                       is.na(season_narm_r$ESA_moisture_sd_region)] <-
  0.01
...
xhats <-
  expand.grid(
    ...
    xhat3 = mean_burial
  ) # AB: predicting air CHELSA_summer_temp at 25% and 75% (assuming you will graph CHELSA_summer_temperature as continuous) but of course you can change this to whatever you want

# Third at CHELSA_summer_tempt - adding CHELSA_summer_temperature levels ----
jags.dat <- list(
  Nobs = nrow(season_narm_r),
  NSite = length(unique(season_narm_r$SiteNum)),
  NRegion = length(unique(season_narm_r$RegionNum)),
...
load("users/hthomas/tea/Stan_outputs/CHELSA_ESA_fits.Rdata")

predsout.space_gridded <- cout[cout$Param %in% c("preds"), ]
predsout.space_gridded$CHELSA_summer_temp <-
  rep(jags.dat$xhat1, each = 2)
predsout.space_gridded$CHELSA_summer_tempBT <-
  predsout.space_gridded$CHELSA_summer_temp + CHELSA_summer_temp_cent_amount
predsout.space_gridded$ESA_moisture <- rep(jags.dat$xhat2, each = 2)
predsout.space_gridded$ESA_moistureBT <-
  predsout.space_gridded$ESA_moisture + ESA_moisture_x_cent_amount
predsout.space_gridded$Tea_TypeNum <-
  rep(c(1, 2), times = (length(predsout.space_gridded$mean) / 2))
predsout.space_gridded$Tea_Type <-
  ifelse(predsout.space_gridded$Tea_TypeNum == 1, "Green", "Rooibos")
...
(
  gridded <- ggplot() +
    geom_point(
      data = season_narm_r_gridded[season_narm_r_gridded$Tea_Type == "Green", ],
      aes(
        x = jitter(CHELSA_summer_temp + CHELSA_summer_temp_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05)
      ),
    ...
    geom_point(
      data = season_narm_r_gridded[season_narm_r_gridded$Tea_Type == "Rooibos", ],
      aes(
        x = jitter(CHELSA_summer_temp + CHELSA_summer_temp_cent_amount, amount = 0.25),
        y = jitter(Loss * 100, amount = 0.05)
      ),
    ...
    geom_ribbon(
      data = predsout.space_gridded[predsout.space_gridded$ESA_moistureBT == LQ_moist |
                                      predsout.space_gridded$ESA_moistureBT == UQ_moist, ],
      aes(
        x = CHELSA_summer_temp + CHELSA_summer_temp_cent_amount,
        ymin = (`2.5%` * 100),
        ymax = (`97.5%` * 100),
        fill = factor(Tea_Type):factor(ESA_moisture)
      ),
      alpha = 0.4
    ) +
    geom_line(
      ...
      aes(
        x = CHELSA_summer_temp + CHELSA_summer_temp_cent_amount,
        y = mean * 100,
        colour = factor(Tea_Type):factor(ESA_moisture)
      ),
      alpha = 0.8,
      lwd = 1.5
    ) +
    ...
    labs(x = "Gridded summer temp. (°C)", y = "Mass loss (%, summer)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
grid.arrange(
  air,
  soil + theme(axis.title.y = element_blank()),
  moisture + theme(axis.title.y = element_blank()),
  CHELSA_summer_temp,
  CHELSA_summer_precip + theme(axis.title.y = element_blank()),
  ESA_moisture + theme(axis.title.y = element_blank()),
  air_moisture,
  soil_moisture + theme(axis.title.y = element_blank()),
  gridded + theme(axis.title.y = element_blank()),
  nrow = 3
)
dev.off()

pdf(file = "users/hthomas/tea/figures/Env_biome_alternative.pdf",
    width = 10.5,
    height = 7.5)
grid.arrange(
  air,
...
pdf(file = "users/hthomas/tea/figures/Env_biome_interactions.pdf",
    width = 8,
    height = 4)
