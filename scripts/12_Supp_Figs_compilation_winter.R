...
load("users/hthomas/tea/Stan_outputs/airtemp_fits_winter.Rdata")
...
(
  air <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(airtemp_mean + airtemp_cent_amount, amount = 0.025),
        y = jitter(Loss * 100, factor = 0.005),
    ...
    labs(x = "Site winter air temp. (°C)", y = "Mass loss (%, winter)") +
...
load("users/hthomas/tea/Stan_outputs/soiltemp_fits_winter.Rdata")
...
(
  soil <- ggplot() +
    geom_point(
      data = season_narm_r_soil,
      aes(
        x = jitter(soiltemp_mean + soiltemp_cent_amount, amount = 0.025),
        y = jitter(Loss * 100, amount = 0.005),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
      alpha = 0.5
    ...
    labs(x = "Site winter soil temp. (°C)", y = "Mass loss (%, winter)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
load("users/hthomas/tea/Stan_outputs/moisture_fits_winter.Rdata")
...
(
  moisture <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(moisture_mean + moisture_cent_amount, amount = 0.025),
        y = jitter(Loss * 100, amount = 0.005),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    labs(x = "Site summer soil moist. (%)", y = "Mass loss (%, winter)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
# season_narm_r$envlevel <-
#   ifelse(
#     season_narm_r$CHELSA_winter_temp_var_level == "Region",
#     0,
#     ifelse(season_narm_r$CHELSA_winter_temp_var_level ==
#              "Site", 1, 2)
#   )
...
# Add env.levels (new - based on nestedness)
env.levels <- season_narm_r %>%
  dplyr::select(CHELSA_winter_temp, ESA_cell, Site, Plot)

season_narm_r$envlevel <- 0

env.levels2 <- env.levels %>%
  dplyr::group_by(ESA_cell) %>%
  dplyr::summarise(n.plots = length(unique(CHELSA_winter_temp)))
...
# AB: REMOVE MISSING VALUES OF CHELSA_winter_temp CHELSA_winter_temp AND TEMPERATURE FOR THE CHELSA_winter_temp X TEMPERATURE INTERACTION MODEL
season_narm_r <-
  season_narm_r[!is.na(season_narm_r$CHELSA_winter_temp), ]
...
# Centre values - AB note: Either don't name this the same thing or save the amount you center by first so we can add it to the xhats later
CHELSA_winter_temp_cent_amount <-
  attr(
    scale(
      season_narm_r$CHELSA_winter_temp,
      center = TRUE,
      scale = FALSE
    ),
    'scaled:center'
...
season_narm_r$CHELSA_winter_temp <-
  scale(season_narm_r$CHELSA_winter_temp,
        center = TRUE,
        scale = FALSE)
days_cent_amount <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
...
season_narm_r_sites <- season_narm_r %>%
  dplyr::group_by(SiteNum) %>%
  dplyr::summarise(
    CHELSA_winter_temp_site = mean(CHELSA_winter_temp),
    CHELSA_winter_temp_sd_site = sd(CHELSA_winter_temp)
  )

season_narm_r$CHELSA_winter_temp_site <-
  season_narm_r_sites$CHELSA_winter_temp_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$CHELSA_winter_temp_sd_site <-
  season_narm_r_sites$CHELSA_winter_temp_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]

season_narm_r$CHELSA_winter_temp_sd_site[season_narm_r$CHELSA_winter_temp_sd_site ==
                                           0] <-
  mean(season_narm_r$CHELSA_winter_temp_sd_site[season_narm_r$CHELSA_winter_temp_sd_site >
                                                  0], na.rm = T)
season_narm_r$CHELSA_winter_temp_sd_site[is.na(season_narm_r$CHELSA_winter_temp_sd_site)] <-
  0.01
...
season_narm_r_regions <- season_narm_r %>%
  dplyr::group_by(RegionNum) %>%
  dplyr::summarise(
    CHELSA_winter_temp_region = mean(CHELSA_winter_temp),
    CHELSA_winter_temp_sd_region = sd(CHELSA_winter_temp)
  )

season_narm_r$CHELSA_winter_temp_region <-
  season_narm_r_regions$CHELSA_winter_temp_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$CHELSA_winter_temp_sd_region <-
  season_narm_r_regions$CHELSA_winter_temp_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]

season_narm_r$CHELSA_winter_temp_sd_region[season_narm_r$CHELSA_winter_temp_sd_region ==
                                             0] <-
  mean(season_narm_r$CHELSA_winter_temp_sd_region[season_narm_r$CHELSA_winter_temp_sd_region >
                                                    0], na.rm = T)
season_narm_r$CHELSA_winter_temp_sd_region[is.na(season_narm_r$CHELSA_winter_temp_sd_region)] <-
  0.01
...
mean_burial <- mean(season_narm_r$Days)
min_CHELSA_winter_temp <-
  min(season_narm_r$CHELSA_winter_temp, na.rm = TRUE)
max_CHELSA_winter_temp <-
  max(season_narm_r$CHELSA_winter_temp, na.rm = TRUE)
min_CHELSA_winter_temp <-
  min(season_narm_r$CHELSA_winter_temp, na.rm = TRUE)
max_CHELSA_winter_temp <-
  max(season_narm_r$CHELSA_winter_temp, na.rm = TRUE)
...
xhats <-
  expand.grid(
    xhat1 = seq(min_CHELSA_winter_temp, max_CHELSA_winter_temp, by = 0.01),
    xhat3 = mean_burial
  ) # AB: predicting CHELSA_winter_temp CHELSA_winter_temp at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
# save key objects
jags.dat_CHELSA_winter_temp <- jags.dat
...
predsout.space_CHELSA_winter_temp <-
  cout[cout$Param %in% c("preds"), ]
predsout.space_CHELSA_winter_temp$CHELSA_winter_temp <-
  rep(jags.dat_CHELSA_winter_temp$xhat1, each = 2)
predsout.space_CHELSA_winter_temp$Temp <-
  rep(jags.dat_CHELSA_winter_temp$xhat2, each = 2)
predsout.space_CHELSA_winter_temp$Tea_TypeNum <-
  rep(c(1, 2), times = (length(predsout.space_CHELSA_winter_temp$mean) /
                          2))
predsout.space_CHELSA_winter_temp$Tea_Type <-
  ifelse(predsout.space_CHELSA_winter_temp$Tea_TypeNum == 1,
         "Green",
         "Rooibos")

season_narm_r_CHELSA_winter_temp <- season_narm_r

# Load figures
(
  CHELSA_winter_temp <- ggplot() +
    geom_point(
      data = season_narm_r_CHELSA_winter_temp,
      aes(
        x = jitter(CHELSA_winter_temp + CHELSA_winter_temp_cent_amount, amount = 0.025),
        y = jitter(Loss * 100, amount = 0.005),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    geom_ribbon(
      data = predsout.space_CHELSA_winter_temp,
      aes(
        x = CHELSA_winter_temp + CHELSA_winter_temp_cent_amount,
        ymin = (`2.5%` * 100),
        ymax = (`97.5%` * 100),
        fill = factor(Tea_Type)
      ),
    ...
    scale_linetype_manual(
      values = c("dashed", "solid"),
      name = "CHELSA_winter_temp",
      labels = c("low", "high")
    ) +
    labs(x = "Gridded winter air temp. (°C)", y = "Mass loss (%, winter)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
# season_narm_r$envlevel <-
#   ifelse(
#     season_narm_r$CHELSA_winter_precip_var_level == "Region",
#     0,
#     ifelse(season_narm_r$CHELSA_winter_precip_var_level ==
#              "Site", 1, 2)
#   )
...
# Add env.levels (new - based on nestedness)
env.levels <- season_narm_r %>%
  dplyr::select(CHELSA_winter_precip, ESA_cell, Site, Plot)

season_narm_r$envlevel <- 0

env.levels2 <- env.levels %>%
  dplyr::group_by(ESA_cell) %>%
  dplyr::summarise(n.plots = length(unique(CHELSA_winter_precip)))
...
# AB: REMOVE MISSING VALUES OF CHELSA_winter_precip CHELSA_winter_precip AND TEMPERATURE FOR THE CHELSA_winter_precip X TEMPERATURE INTERACTION MODEL
season_narm_r <-
  season_narm_r[!is.na(season_narm_r$CHELSA_winter_precip), ]
...
# Centre values - AB note: Either don't name this the same thing or save the amount you center by first so we can add it to the xhats later
CHELSA_winter_precip_cent_amount <-
  attr(
    scale(
      season_narm_r$CHELSA_winter_precip,
      center = TRUE,
      scale = FALSE
    ),
    'scaled:center'
...
season_narm_r$CHELSA_winter_precip <-
  scale(season_narm_r$CHELSA_winter_precip,
        center = TRUE,
        scale = FALSE)
days_cent_amount <-
  attr(scale(season_narm_r$Days, center = TRUE, scale = FALSE),
...
season_narm_r_sites <- season_narm_r %>%
  dplyr::group_by(SiteNum) %>%
  dplyr::summarise(
    CHELSA_winter_precip_site = mean(CHELSA_winter_precip),
    CHELSA_winter_precip_sd_site = sd(CHELSA_winter_precip)
  )

season_narm_r$CHELSA_winter_precip_site <-
  season_narm_r_sites$CHELSA_winter_precip_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$CHELSA_winter_precip_sd_site <-
  season_narm_r_sites$CHELSA_winter_precip_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]

season_narm_r$CHELSA_winter_precip_sd_site[season_narm_r$CHELSA_winter_precip_sd_site ==
                                             0] <-
  mean(season_narm_r$CHELSA_winter_precip_sd_site[season_narm_r$CHELSA_winter_precip_sd_site >
                                                    0], na.rm = T)
season_narm_r$CHELSA_winter_precip_sd_site[is.na(season_narm_r$CHELSA_winter_precip_sd_site)] <-
  0.01
...
season_narm_r_regions <- season_narm_r %>%
  dplyr::group_by(RegionNum) %>%
  dplyr::summarise(
    CHELSA_winter_precip_region = mean(CHELSA_winter_precip),
    CHELSA_winter_precip_sd_region = sd(CHELSA_winter_precip)
  )

season_narm_r$CHELSA_winter_precip_region <-
  season_narm_r_regions$CHELSA_winter_precip_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$CHELSA_winter_precip_sd_region <-
  season_narm_r_regions$CHELSA_winter_precip_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]

season_narm_r$CHELSA_winter_precip_sd_region[season_narm_r$CHELSA_winter_precip_sd_region ==
                                               0] <-
  mean(season_narm_r$CHELSA_winter_precip_sd_region[season_narm_r$CHELSA_winter_precip_sd_region >
                                                      0], na.rm = T)
season_narm_r$CHELSA_winter_precip_sd_region[is.na(season_narm_r$CHELSA_winter_precip_sd_region)] <-
  0.01
...
mean_burial <- mean(season_narm_r$Days)
min_CHELSA_winter_precip <-
  min(season_narm_r$CHELSA_winter_precip, na.rm = TRUE)
max_CHELSA_winter_precip <-
  max(season_narm_r$CHELSA_winter_precip, na.rm = TRUE)
min_CHELSA_winter_precip <-
  min(season_narm_r$CHELSA_winter_precip, na.rm = TRUE)
max_CHELSA_winter_precip <-
  max(season_narm_r$CHELSA_winter_precip, na.rm = TRUE)
...
xhats <-
  expand.grid(
    xhat1 = seq(min_CHELSA_winter_precip, max_CHELSA_winter_precip, by = 0.01),
    xhat3 = mean_burial
  ) # AB: predicting CHELSA_winter_precip CHELSA_winter_precip at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
# save key objects
jags.dat_CHELSA_winter_precip <- jags.dat

# Load figure data

load("users/hthomas/tea/Stan_outputs/CHELSA_winter_precip_fits_winter.Rdata")

subset(cout, Param == "gamma0")
subset(cout, Param == "gamma1")

predsout.space_CHELSA_winter_precip <-
  cout[cout$Param %in% c("preds"), ]
predsout.space_CHELSA_winter_precip$CHELSA_winter_precip <-
  rep(jags.dat_CHELSA_winter_precip$xhat1, each = 2)
predsout.space_CHELSA_winter_precip$Temp <-
  rep(jags.dat_CHELSA_winter_precip$xhat2, each = 2)
predsout.space_CHELSA_winter_precip$Tea_TypeNum <-
  rep(c(1, 2), times = (length(
    predsout.space_CHELSA_winter_precip$mean
  ) / 2))
predsout.space_CHELSA_winter_precip$Tea_Type <-
  ifelse(predsout.space_CHELSA_winter_precip$Tea_TypeNum == 1,
         "Green",
         "Rooibos")

season_narm_r_CHELSA_winter_precip <- season_narm_r

# Load figures
(
  CHELSA_winter_precip <- ggplot() +
    geom_point(
      data = season_narm_r_CHELSA_winter_precip,
      aes(
        x = jitter(
          CHELSA_winter_precip + CHELSA_winter_precip_cent_amount,
          amount = 0.01
        ),
        y = jitter(Loss * 100, amount = 0.005),
        colour = factor(Tea_Type)
    ...
    geom_ribbon(
      data = predsout.space_CHELSA_winter_precip,
      aes(
        x = CHELSA_winter_precip + CHELSA_winter_precip_cent_amount,
        ymin = (`2.5%` * 100),
        ymax = (`97.5%` * 100),
        fill = factor(Tea_Type)
      ),
    ...
    scale_linetype_manual(
      values = c("dashed", "solid"),
      name = "CHELSA_winter_precip",
      labels = c("low", "high")
    ) +
    labs(x = "Gridded winter precip. (mm)", y = "Mass loss (%, winter)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
season_narm <- winter %>%
  filter(is.finite(winter[, var.num]), is.finite(ESA_moisture))
...
load("users/hthomas/tea/Stan_outputs/ESA_moisture_winter_fits_winter.Rdata")
...
(
  ESA_moisture <- ggplot() +
    geom_point(
      ...
      aes(
        x = jitter(ESA_moisture + ESA_moisture_cent_amount, amount = 0.025),
        y = jitter(Loss * 100, amount = 0.005),
        colour = factor(Tea_Type)
      ),
      pch = 16 ,
    ...
    labs(x = "Gridded summer soil moist. (%)", y = "Mass loss (%, winter)") +
    theme(legend.position = "none") +
    theme(
      title = element_text(size = 15),
      axis.text = element_text(size = 13),
...
pdf(file = "users/hthomas/tea/figures/Env_biome_winter.pdf",
    width = 10.5,
    height = 7.5)
grid.arrange(air,
             soil,
