...
season_narm<-summer %>%
  dplyr::filter(is.finite(summer[,var.num]),is.finite(moisture_mean))
...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  dplyr::summarise(moisture_mean_site = mean(moisture_mean),
...
season_narm_r_regions<-season_narm_r %>%
  ...
  dplyr::summarise(moisture_mean_region = mean(moisture_mean),
...
write("
      ...
      real<lower=-2,upper=2> gamma5[NTea];  // polynomial relationship with moisture
      ...
      else aMeanRegion[i] ~ normal(gamma0[tea_type_region[i]] + gamma1[tea_type_region[i]]*temp_pred_region[i] + gamma4[tea_type_region[i]]*days_pred_region[i] + gamma5[tea_type_region[i]]*square(temp_pred_region[i])  , sigma_overall); 
      }
      ...
      ","Stan_scripts/moisture_loss_3.stan")



stanc('Stan_scripts/moisture_loss_3.stan') #check model
...
fit_space <- stan(file = 'Stan_scripts/moisture_loss_3.stan', 
                  data = jags.dat, 
                  init=inits, 
                  iter = 15000, 
                  chains = 2, 
...
print(fit_space)
stan_trace(fit_space, inc_warmup = TRUE, pars = c("gamma0","gamma1","gamma5"))
...
saveRDS(fixed_effects,file = file.path("Stan_outputs","parameter_sample","moisture_summer_parameter_poly.RData"))
...
save(predsout.space, file = "Stan_outputs/moisture_preds_summer_poly.Rdata")
save(cout, file = "Stan_outputs/moisture_fits_summer_poly.Rdata")
...
ggplot()+
  ...
  geom_point(data=season_narm_r[season_narm_r$Region != "Svalbard",],aes(x=moisture_mean+moisture_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_point(data=season_narm_r[season_narm_r$Region == "Svalbard",],aes(x=moisture_mean+moisture_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =21 ,alpha=0.6)+
  geom_smooth(data=season_narm_r[season_narm_r$Region == "Svalbard",],aes(x=moisture_mean+moisture_cent_amount,y=Loss,colour=factor(Tea_Type))) +
