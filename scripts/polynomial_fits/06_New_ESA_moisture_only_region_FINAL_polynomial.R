...
season_narm<-summer %>%
  dplyr::filter(is.finite(summer[,var.num]),is.finite(ESA_moisture))
...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  dplyr::summarise(ESA_moisture_site = mean(ESA_moisture),
...
season_narm_r_regions<-season_narm_r %>%
  group_by(RegionNum) %>%
  dplyr::summarise(ESA_moisture_region = mean(ESA_moisture),
...
write("
      ...
      real<lower=-2,upper=2> gamma5[NTea];  // polynomial relationship with ESA_moisture
      ...
      ","Stan_scripts/ESA_moisture_loss_3.stan")



stanc('Stan_scripts/ESA_moisture_loss_3.stan') #check model
...
fit_space <- stan(file = 'Stan_scripts/ESA_moisture_loss_3.stan', data = jags.dat, init=inits, iter = 25000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
print(fit_space)
stan_trace(fit_space, inc_warmup = TRUE, pars = c("gamma0","gamma5"))
...
tmp <- rep(names(fixed_effects),each = 25000)
...
saveRDS(fixed_effects,file = file.path("Stan_outputs","parameter_sample","ESA_moist_parameter_poly.RData"))
...
save(predsout.space, file = "Stan_outputs/ESA_moisture_preds_summer_poly.Rdata")
save(cout, file = "ESA_moisture_fits_summer_poly.Rdata")
