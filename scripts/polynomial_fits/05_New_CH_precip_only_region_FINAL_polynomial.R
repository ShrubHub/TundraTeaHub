...
season_narm<-summer %>%
  dplyr::filter(is.finite(summer[,var.num]),is.finite(CHELSA_summer_precip))
...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  dplyr::summarise(CHELSA_summer_precip_site = mean(CHELSA_summer_precip),
...
season_narm_r_regions<-season_narm_r %>%
  group_by(RegionNum) %>%
  dplyr::summarise(CHELSA_summer_precip_region = mean(CHELSA_summer_precip),
...
write("
      ...
      real<lower=-2,upper=2> gamma5[NTea];  // polynomial relationship with CHELSA_summer_precip
      ...
      ","Stan_scripts/CHELSA_summer_precip_loss_3.stan")



stanc('Stan_scripts/CHELSA_summer_precip_loss_3.stan') #check model
...
fit_space <- stan(file = 'Stan_scripts/CHELSA_summer_precip_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
sum(param_dt$poly_rooibos<0)/length(param_dt$poly_rooibos)
...
saveRDS(fixed_effects,file = file.path("Stan_outputs","parameter_sample","CH_precip_parameter_poly.RData"))
...
save(predsout.space, file = "Stan_outputs/CHELSA_summer_precip_preds_summer_poly.Rdata")
save(cout, file = "Stan_outputs/CHELSA_summer_precip_fits_summer_poly.Rdata")
