...
write("
      ...
      real<lower=-2,upper=2> gamma5[NTea];  // slope of squared airtemp
      ...
      as[i] ~ normal(gamma0[tea_type_site[i]] + gamma1[tea_type_site[i]]*temp_pred_site[i] + gamma4[tea_type_site[i]]*days_pred_site[i]  + gamma5[tea_type_site[i]]*square(temp_pred_site[i]) , sigma_overall); 
      ...
      else aMeanRegion[i] ~ normal(gamma0[tea_type_region[i]] + gamma1[tea_type_region[i]]*temp_pred_region[i] + gamma4[tea_type_region[i]]*days_pred_region[i]  + gamma5[tea_type_region[i]]*square(days_pred_region[i] ) , sigma_overall); 
      }
...
print(fit_space)
stan_trace(fit_space, inc_warmup = TRUE, pars = c("gamma4"))

stan_trace(fit_space, inc_warmup = TRUE, pars = c("aMeanSite[1]","aMeanSite[2]"))

library(data.table)

param_dt <- data.table(poly_green = fit_space@sim$samples[[1]]$`gamma5[1]`,
                       poly_rooibos = fit_space@sim$samples[[1]]$`gamma5[2]`)

ggplot(param_dt[],aes(x = poly_green))+
  geom_histogram(fill="cadetblue",color="grey20",origin=0)+
  theme_bw()+
  scale_x_continuous(limits = c(-0.003,0.003) )


fixed_effects <- fit_space@sim$samples[[1]][grepl("gamma",names(fit_space@sim$samples[[1]]))]
names(fixed_effects) <- str_replace(names(fixed_effects),"\\[1\\]","_green")
names(fixed_effects) <- str_replace(names(fixed_effects),"\\[2\\]","_rooibos")
tmp <- rep(names(fixed_effects),each = 15000)
fixed_effects <- do.call(c,fixed_effects)
fixed_effects <- data.table(parameter = fixed_effects, name = tmp)
fixed_effects <- fixed_effects[!grepl("[23]",name),]
saveRDS(fixed_effects,file = file.path("Stan_outputs","parameter_sample","airtemp_sites_parameter_poly.RData"))
...
cout_dt[Param %in% c("gamma0","gamma1","gamma2","gamma3","gamma5"),]
...
quantile(fixed_effects[name == "gamma5_rooibos",]$parameter,probs=c(0.025,0.975),na.action="ignore")
...
region.compare <- ddply(season_narm_r, c("RegionNum","Tea_Type"), dplyr::summarise,
                        rawLoss = mean(Loss))
...
save(predsout.space, file = "Stan_outputs/airtemp_only_preds_summer_poly.Rdata")
save(cout, file = "Stan_outputs/airtemp_only_fits_summer_poly.Rdata")
...
ggplot()+
 ...
 # geom_point(data=season_narm_r[season_narm_r$Region != "Svalbard",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6,position = position_jitter(width = 0.1))+
 # geom_point(data=season_narm_r[season_narm_r$Region == "Svalbard",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =21 ,alpha=1,position = position_jitter(width = 0.1))+
  
  geom_line(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
  coord_cartesian(y = c(0,1))+
  scale_colour_manual(values = c("#8B2323","#006400")[c(2,1)], name = "Tea Type")+
  scale_fill_manual(values = c("#8B2323","#006400")[c(2,1)], name = "Tea Type")+
...
(airtemp_only_summer<-ggplot()+
    ...
    geom_ribbon(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    #eom_point(data=season_narm_r,aes(x=airtemp_mean+airtemp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6,position = position_jitter(width = 0.1))+
    geom_line(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
    coord_cartesian(y = c(0,1))+
    scale_colour_manual(values = c("#8B2323","#006400"),breaks =c("Rooibos","Green"), name = "Tea Type")+
    scale_fill_manual(values = c("#8B2323","#006400"), breaks =c("Rooibos","Green"),name = "Tea Type")+
    scale_linetype_manual(values = c("dashed","solid"), name = "airtemp", labels = c("low","high"))+
    labs(x = "Air Temperature (°C)", y = "Mass Loss (%)")+
    theme(legend.position = "none"))
