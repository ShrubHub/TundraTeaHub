...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  dplyr::summarise(soiltemp_mean_site = mean(soiltemp_mean),
...
season_narm_r_regions<-season_narm_r %>%
  group_by(RegionNum) %>%
  dplyr::summarise(soiltemp_mean_region = mean(soiltemp_mean),
...
write("
      ...
      real<lower=-2,upper=2> gamma5[NTea];  // polynomial relationship with soil temperature
      ...
      ","Stan_scripts/soiltemp_loss_3.stan")



stanc('Stan_scripts/soiltemp_loss_3.stan') #check model
...
fit_space <- stan(file = 'Stan_scripts/soiltemp_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
saveRDS(fixed_effects,file = file.path("Stan_outputs","parameter_sample","soiltemp_summer_parameter_poly.RData"))
...
cout[cout$Param %in% c("gamma0","gamma1","gamma2","gamma5"),] #these will tell you about the "significance" of your environmental predictors
...
save(predsout.space, file = "Stan_outputs/soiltemp_preds_summer_poly.Rdata")
save(cout, file = "Stan_outputs/soiltemp_fits_summer_poly.Rdata")
